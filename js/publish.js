/* Real one-click publish: commits the Admin's applied data straight to the
   ...
   orphaned objects GitHub garbage-collects. */
(function () {
  ...
  /* meta: { asOnDate, rowCount, regions, commitMessage, publishedBy, isRollback } */
  async function publishData(dataJsonString, meta, onProgress) {
    meta = meta || {};
    ...
    progress('Published.');
    return { commitSha: newCommit.sha, historyFile: historyFileName };
  }

  window.UPGBPublish = { publishData, getHistoryIndex, getHistoryFileContent };
})();
