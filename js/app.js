...
function pendingUnpublishedLabel(){
  ...
  if(typeof __pendingPnpaDailyData!=='undefined' && __pendingPnpaDailyData) parts.push('the PNPA Reports (Daily) upload');
  if(typeof __pendingPnpaWeeklyData!=='undefined' && __pendingPnpaWeeklyData) parts.push('the PNPA Reports (Weekly) upload');
  if(typeof __pendingPnpaMonthlyData!=='undefined' && __pendingPnpaMonthlyData) parts.push('the PNPA Reports (Monthly) upload');
  if(typeof __pendingRctEfficiencyData!=='undefined' && __pendingRctEfficiencyData) parts.push('the RCT Efficiency upload');
  if(typeof __pendingKccRenewalData!=='undefined' && __pendingKccRenewalData) parts.push('the KCC Renewal upload');
  if(typeof __pendingKccRolloverData!=='undefined' && __pendingKccRolloverData) parts.push('the KCC Rollover upload');
  return parts;
}
...
function openPublishReview(){
  ...
  let pnpaDailyLabel = null, pnpaWeeklyLabel = null, pnpaMonthlyLabel = null, rctLabel = null, kccRenewalLabel = null, kccRolloverLabel = null;
  if(__pendingPnpaDailyData){
    const d = esc(fmtDate(isoToDate(__pendingPnpaDailyData.asOnDate)));
    pnpaDailyLabel = `PNPA Reports — Daily (${__pendingPnpaDailyData.regions.length} regions, as on ${d})`;
    items.push(publishReviewItemRow({ icon: ICON_ALERT_CIRCLE, title: 'PNPA Reports — Daily', sub: `${__pendingPnpaDailyData.regions.length} regions · as on ${d}` }));
  }
  if(__pendingPnpaWeeklyData){
    const d = esc(fmtDate(isoToDate(__pendingPnpaWeeklyData.asOnDate)));
    pnpaWeeklyLabel = `PNPA Reports — Weekly (${__pendingPnpaWeeklyData.regions.length} regions, as on ${d})`;
    items.push(publishReviewItemRow({ icon: ICON_ALERT_CIRCLE, title: 'PNPA Reports — Weekly', sub: `${__pendingPnpaWeeklyData.regions.length} regions · as on ${d}` }));
  }
  if(__pendingPnpaMonthlyData){
    const d = esc(fmtDate(isoToDate(__pendingPnpaMonthlyData.asOnDate)));
    pnpaMonthlyLabel = `PNPA Reports — Monthly (${__pendingPnpaMonthlyData.regions.length} regions, as on ${d})`;
    items.push(publishReviewItemRow({ icon: ICON_ALERT_CIRCLE, title: 'PNPA Reports — Monthly', sub: `${__pendingPnpaMonthlyData.regions.length} regions · as on ${d}` }));
  }
  if(__pendingRctEfficiencyData){
    const d = esc(fmtDate(isoToDate(__pendingRctEfficiencyData.asOnDate)));
    rctLabel = `RCT Efficiency (${__pendingRctEfficiencyData.regions.length} regions, as on ${d})`;
    items.push(publishReviewItemRow({ icon: ICON_TARGET, title: 'RCT Efficiency', sub: `${__pendingRctEfficiencyData.regions.length} regions · as on ${d}` }));
  }
  if(__pendingKccRenewalData){
    const d = esc(fmtDate(isoToDate(__pendingKccRenewalData.asOnDate)));
    kccRenewalLabel = `KCC Renewal (${__pendingKccRenewalData.regions.length} regions, as on ${d})`;
    items.push(publishReviewItemRow({ icon: ICON_TARGET, title: 'KCC Renewal', sub: `${__pendingKccRenewalData.regions.length} regions · as on ${d}` }));
  }
  if(__pendingKccRolloverData){
    const d = esc(fmtDate(isoToDate(__pendingKccRolloverData.asOnDate)));
    kccRolloverLabel = `KCC Rollover (${__pendingKccRolloverData.regions.length} regions, as on ${d})`;
    items.push(publishReviewItemRow({ icon: ICON_TARGET, title: 'KCC Rollover', sub: `${__pendingKccRolloverData.regions.length} regions · as on ${d}` }));
  }
  ...
  __pendingPublish = {
    ...
    labels: { bankLabel, pnpaLabel, kccovLabel, pnpaDailyLabel, pnpaWeeklyLabel, pnpaMonthlyLabel, rctLabel, kccRenewalLabel, kccRolloverLabel },
  };
...
async function confirmPublish(){
  ...
  try{
    ...
    if(__pendingPublish.type!=='rollback' && __pendingPnpaDailyData){
      extraFiles = (extraFiles||[]).concat([{ path:'data/pnpa-daily.json', content: __pendingPnpaDailyData, label: labels.pnpaDailyLabel }]);
    }
    if(__pendingPublish.type!=='rollback' && __pendingPnpaWeeklyData){
      extraFiles = (extraFiles||[]).concat([{ path:'data/pnpa-weekly.json', content: __pendingPnpaWeeklyData, label: labels.pnpaWeeklyLabel }]);
    }
    if(__pendingPublish.type!=='rollback' && __pendingPnpaMonthlyData){
      extraFiles = (extraFiles||[]).concat([{ path:'data/pnpa-monthly.json', content: __pendingPnpaMonthlyData, label: labels.pnpaMonthlyLabel }]);
    }
    if(__pendingPublish.type!=='rollback' && __pendingRctEfficiencyData){
      extraFiles = (extraFiles||[]).concat([{ path:'data/rct-efficiency.json', content: __pendingRctEfficiencyData, label: labels.rctLabel }]);
    }
    if(__pendingPublish.type!=='rollback' && __pendingKccRenewalData){
      extraFiles = (extraFiles||[]).concat([{ path:'data/kcc-renewal.json', content: __pendingKccRenewalData, label: labels.kccRenewalLabel }]);
    }
    if(__pendingPublish.type!=='rollback' && __pendingKccRolloverData){
      extraFiles = (extraFiles||[]).concat([{ path:'data/kcc-rollover.json', content: __pendingKccRolloverData, label: labels.kccRolloverLabel }]);
    }
    ...
    __pendingPnpaDailyData = null;
    __pendingPnpaWeeklyData = null;
    __pendingPnpaMonthlyData = null;
    __pendingRctEfficiencyData = null;
    __pendingKccRenewalData = null;
    __pendingKccRolloverData = null;
    closePublishReview();
    loadVersionHistory();
  } catch(err){
...
/* ---------- Client-side PDF parsers for the 6 PNPA/KCC reports ----------
   All six share one shape: numbered region rows (S.No, Region, Branches,
   N numeric fields) grouped across 3 pages (Gorakhpur/Lucknow/Moradabad),
   an area-subtotal row per page, and a final grand-total row. Reuses the
   same text-clustering technique as parseBankPdf()/bankPdfClusterRows.
   KCC Rollover is the one exception -- a single flat 65-row rank list
   (not grouped by area per row) followed by a separate 3-row area-summary
   table on the same page, so it gets its own dedicated parser below. */
function rtPdfNum(s){ const c = String(s).replace(/%/g,'').replace(/,/g,'').trim(); const n = parseFloat(c); return isNaN(n)?null:n; }
function rtPdfExtractDate(pageOneText){
  const m = /(\d{1,2})[.\-](\d{1,2})[.\-](\d{4})/.exec(pageOneText);
  if(!m) return null;
  return `${m[3]}-${String(m[2]).padStart(2,'0')}-${String(m[1]).padStart(2,'0')}`;
}
async function parseRegionReportPdf(arrayBuffer, opts){
  // opts: {nfields, areaTotalPattern(str)=>areaNameOrNull, grandTotalPattern(str)=>bool}
  if(!window.pdfjsLib) throw new Error('PDF reader did not load — check your connection and reload the page, then try again.');
  window.pdfjsLib.GlobalWorkerOptions.workerSrc = 'js/vendor/pdf.worker.min.js';
  const doc = await window.pdfjsLib.getDocument({ data: arrayBuffer }).promise;
  let regions = [], areaTotals = {}, grandTotal = null, asOnDate = null, currentArea = null, pending = [];
  for(let p=1; p<=doc.numPages; p++){
    const page = await doc.getPage(p);
    const content = await page.getTextContent();
    const items = content.items.map(it=>({x:it.transform[4], y:it.transform[5], str:(it.str||'').trim()})).filter(it=>it.str!=='');
    const rows = bankPdfClusterRows(items, 3.5);
    if(p===1) asOnDate = rtPdfExtractDate(rows.slice(0,4).map(r=>r.map(it=>it.str).join(' ')).join(' '));
    for(const row of rows){
      let strs = row.map(it=>it.str);
      if(!strs.length) continue;
      /* The page's rotated side-margin area label ("GORAKHPUR"/"LUCKNOW"/
         "MORADABAD", printed vertically along the left edge) occasionally
         lands within the row-clustering tolerance of one specific data
         row's y-coordinate, prepending a stray token ahead of the real
         S.No. Strip it whenever it's immediately followed by what's
         otherwise a normal region row. */
      if(/^(GORAKHPUR|LUCKNOW|MORADABAD)$/i.test(strs[0]) && /^\d+$/.test(strs[1]) && strs.length===opts.nfields+4){
        strs = strs.slice(1);
      }
      if(/^\d+$/.test(strs[0]) && strs.length===opts.nfields+3){
        const nums = strs.slice(-opts.nfields).map(rtPdfNum);
        const branches = parseInt(strs[strs.length-opts.nfields-1], 10);
        const region = strs.slice(1, strs.length-opts.nfields-1).join(' ');
        pending.push({ sno: parseInt(strs[0],10), region, branches, nums });
        continue;
      }
      const areaName = opts.areaTotalPattern(strs[0]);
      if(areaName && strs.length===opts.nfields+2){
        currentArea = areaName;
        pending.forEach(r=>{ r.area = currentArea; });
        regions.push(...pending);
        areaTotals[areaName] = { branches: parseInt(strs[1],10), nums: strs.slice(-opts.nfields).map(rtPdfNum) };
        pending = [];
        continue;
      }
      if(opts.grandTotalPattern(strs[0]) && strs.length>=3){
        /* Front-anchored (not back-anchored like region/area rows): the
           grand-total row for KCC Renewal has no trailing "rank" value
           (rank is meaningless for a sum), one field short of nfields --
           slicing from the back would shift every field left by one and
           silently swallow the branches count into the first field. */
        const rawNums = strs.slice(2).map(rtPdfNum);
        while(rawNums.length < opts.nfields) rawNums.push(null);
        grandTotal = { branches: parseInt(strs[1],10), nums: rawNums.slice(0, opts.nfields) };
      }
    }
  }
  if(!grandTotal || regions.length<50){
    throw new Error('Could not recognize this PDF\'s layout — expected a region-wise report with 65 regions and a grand total row.');
  }
  const warning = grandTotal.branches && regions.length < 65
    ? `Parsed ${regions.length} of 65 regions — the source PDF likely has a formatting glitch on the missing row(s) (Head Office's export sometimes merges two numbers together). Numbers you can see below are accurate; double-check the missing region(s) against the PDF before publishing.`
    : null;
  return { asOnDate, regions, areaTotals, grandTotal, warning };
}
function rtNamedRows(regions, fieldNames){
  return regions.map(r=>{
    const row = { sno:r.sno, region:r.region, branches:r.branches, area:r.area };
    fieldNames.forEach((f,i)=>{ row[f]=r.nums[i]; });
    return row;
  });
}
function rtNamedTotal(total, fieldNames, label){
  const row = { branches: total.branches };
  if(label) row.label = label;
  fieldNames.forEach((f,i)=>{ row[f]=total.nums[i]; });
  return row;
}
const RT_AREA_LABELS_GKP_LKO_MBD = (s)=>({'GKP TOTAL':'Gorakhpur','LKO TOTAL':'Lucknow','MBD TOTAL':'Moradabad'}[s]||null);
const RT_AREA_LABELS_NAME_TOTAL = (s)=>({'Gorakhpur Total':'Gorakhpur','Lucknow Total':'Lucknow','Moradabad Total':'Moradabad','Camp Offfice Gorakhpur Total':'Gorakhpur','Camp Office Gorakhpur Total':'Gorakhpur'}[s]||null);
const RT_AREA_LABELS_NAME_ONLY = (s)=>({'Gorakhpur':'Gorakhpur','Lucknow':'Lucknow','Moradabad':'Moradabad'}[s]||null);
const RT_GRAND_UPGB = (s)=>/^UPGB (Grand Total|TOTAL|Total)$/i.test(s.trim());

const PNPA_DAILY_FIELDS = ['totalAcc','totalAmt','kccAcc','kccAmt','tlAcc','tlAmt','ccodAcc','ccodAmt','nonfinAcc','nonfinAmt','custAcc','custAmt'];
const PNPA_MONTHLY_FIELDS = ['pnpaAug','remainingPnpa','turnedToNpa','reduction','pctRed','kccAug','kccRemaining','kccTurned','kccReduction','pctNpaKcc','pctRedKcc','tlAug','tlRemaining','pctRedTl','ccodAug','ccodRemaining','pctRedCcod','nonfinAug','nonfinRemaining','pctRedNonfin'];
const RCT_FIELDS = ['rolloverMar','rolloverJul','rolloverAug','collectionMar','collectionJul','collectionAug','turnoverMar','turnoverJul','turnoverAug'];
const RENEWAL_FIELDS = ['dueAcc','dueAmt','renewedAcc','renewedAmt','pctRenewal','turnedAcc','turnedAmt','pendingAcc','pendingAmt','lastDayAcc','lastDayAmt','rank'];

async function parsePnpaDailyPdf(buf){
  const r = await parseRegionReportPdf(buf, {nfields:12, areaTotalPattern:RT_AREA_LABELS_GKP_LKO_MBD, grandTotalPattern:RT_GRAND_UPGB});
  return { asOnDate:r.asOnDate, regions:rtNamedRows(r.regions, PNPA_DAILY_FIELDS),
    areaTotals: Object.fromEntries(Object.entries(r.areaTotals).map(([k,v])=>[k, rtNamedTotal(v, PNPA_DAILY_FIELDS)])),
    grandTotal: rtNamedTotal(r.grandTotal, PNPA_DAILY_FIELDS), warning: r.warning };
}
async function parsePnpaWeeklyPdf(buf){
  const r = await parseRegionReportPdf(buf, {nfields:12, areaTotalPattern:RT_AREA_LABELS_GKP_LKO_MBD, grandTotalPattern:RT_GRAND_UPGB});
  return { asOnDate:r.asOnDate, weekLabel:'', regions:rtNamedRows(r.regions, PNPA_DAILY_FIELDS),
    areaTotals: Object.fromEntries(Object.entries(r.areaTotals).map(([k,v])=>[k, rtNamedTotal(v, PNPA_DAILY_FIELDS)])),
    grandTotal: rtNamedTotal(r.grandTotal, PNPA_DAILY_FIELDS), warning: r.warning };
}
async function parsePnpaMonthlyPdf(buf){
  const r = await parseRegionReportPdf(buf, {nfields:20, areaTotalPattern:RT_AREA_LABELS_NAME_ONLY, grandTotalPattern:RT_GRAND_UPGB});
  return { asOnDate:r.asOnDate, month:'', monthLabel:'', regions:rtNamedRows(r.regions, PNPA_MONTHLY_FIELDS),
    areaTotals: Object.fromEntries(Object.entries(r.areaTotals).map(([k,v])=>[k, rtNamedTotal(v, PNPA_MONTHLY_FIELDS)])),
    grandTotal: rtNamedTotal(r.grandTotal, PNPA_MONTHLY_FIELDS, 'UPGB Total'), warning: r.warning };
}
async function parseRctEfficiencyPdf(buf){
  const r = await parseRegionReportPdf(buf, {nfields:9, areaTotalPattern:RT_AREA_LABELS_NAME_TOTAL, grandTotalPattern:RT_GRAND_UPGB});
  return { asOnDate:r.asOnDate, dates:[], regions:rtNamedRows(r.regions, RCT_FIELDS),
    grandTotal: rtNamedTotal(r.grandTotal, RCT_FIELDS, 'UPGB Total'), warning: r.warning };
}
async function parseKccRenewalPdf(buf){
  const r = await parseRegionReportPdf(buf, {nfields:12, areaTotalPattern:RT_AREA_LABELS_NAME_TOTAL, grandTotalPattern:RT_GRAND_UPGB});
  // The grand-total row has no rank (12th field absent -> only 11 numbers after branches); re-slice defensively.
  return { asOnDate:r.asOnDate, fyLabel:'FY 26-27', regions:rtNamedRows(r.regions, RENEWAL_FIELDS),
    grandTotal: rtNamedTotal(r.grandTotal, RENEWAL_FIELDS, 'UPGB Total'), warning: r.warning };
}

/* KCC Rollover -- flat 65-row rank list (region names in the second
   column, not grouped per-row by area) followed on the same page by a
   3-row GKP/LKO/MBD area-summary block using the SAME leading-digit
   shape (1/2/3), so those three rows are excluded from the region list
   by name rather than by position. */
const ROLLOVER_FIELDS = ['stdAcc','stdAmt','rolloverTotalAcc','rolloverTotalAmt','rolloverMar27Acc','rolloverMar27Amt','pctRollover','prevTotalAcc','prevTotalAmt','prevMar27Acc','prevMar27Amt'];
async function parseKccRolloverPdf(arrayBuffer){
  if(!window.pdfjsLib) throw new Error('PDF reader did not load — check your connection and reload the page, then try again.');
  window.pdfjsLib.GlobalWorkerOptions.workerSrc = 'js/vendor/pdf.worker.min.js';
  const doc = await window.pdfjsLib.getDocument({ data: arrayBuffer }).promise;
  const nfields = 11;
  let regions = [], areaTotals = {}, grandTotal = null, asOnDate = null;
  for(let p=1; p<=doc.numPages; p++){
    const page = await doc.getPage(p);
    const content = await page.getTextContent();
    const items = content.items.map(it=>({x:it.transform[4], y:it.transform[5], str:(it.str||'').trim()})).filter(it=>it.str!=='');
    const rows = bankPdfClusterRows(items, 3.5);
    if(p===1) asOnDate = rtPdfExtractDate(rows.slice(0,3).map(r=>r.map(it=>it.str).join(' ')).join(' '));
    for(const row of rows){
      const strs = row.map(it=>it.str);
      if(!strs.length) continue;
      if(/^\d+$/.test(strs[0]) && strs.length===nfields+3){
        const region = strs[1];
        if(region==='GKP' || region==='LKO' || region==='MBD'){
          areaTotals[region] = { branches: parseInt(strs[2],10), nums: strs.slice(-nfields).map(rtPdfNum) };
          continue;
        }
        const nums = strs.slice(-nfields).map(rtPdfNum);
        const branches = parseInt(strs[strs.length-nfields-1], 10);
        regions.push({ sno: parseInt(strs[0],10), rank: parseInt(strs[0],10), region, branches, nums });
        continue;
      }
      if(RT_GRAND_UPGB(strs[0]) && strs.length>=nfields+1){
        grandTotal = { branches: parseInt(strs[1],10), nums: strs.slice(-nfields).map(rtPdfNum) };
      }
    }
  }
  if(!grandTotal || regions.length<50){
    throw new Error('Could not recognize this PDF\'s layout — expected the Region-wise KCC Rollover Progress report with 65 regions and a UPGB TOTAL row.');
  }
  const areaNameMap = {GKP:'Gorakhpur', LKO:'Lucknow', MBD:'Moradabad'};
  const warning = regions.length < 65
    ? `Parsed ${regions.length} of 65 regions — the source PDF likely has a formatting glitch on the missing row(s) (Head Office's export sometimes merges two numbers together). Numbers you can see below are accurate; double-check the missing region(s) against the PDF before publishing.`
    : null;
  return {
    asOnDate,
    ...
    regions: regions.map(r=>{
      const row = { sno:r.sno, rank:r.rank, region: r.region.length>2 ? (r.region[0]+r.region.slice(1).toLowerCase()) : r.region, branches:r.branches };
      ROLLOVER_FIELDS.forEach((f,i)=>{ row[f]=r.nums[i]; });
      return row;
    }),
    areaTotals: Object.fromEntries(Object.entries(areaTotals).map(([k,v])=>[areaNameMap[k]||k, rtNamedTotal(v, ROLLOVER_FIELDS)])),
    grandTotal: rtNamedTotal(grandTotal, ROLLOVER_FIELDS, 'UPGB Total'),
  };
}

window.parsePnpaDailyPdf = parsePnpaDailyPdf;
window.parsePnpaWeeklyPdf = parsePnpaWeeklyPdf;
window.parsePnpaMonthlyPdf = parsePnpaMonthlyPdf;
window.parseRctEfficiencyPdf = parseRctEfficiencyPdf;
window.parseKccRenewalPdf = parseKccRenewalPdf;
window.parseKccRolloverPdf = parseKccRolloverPdf;

/* ---------- PNPA Reports (Daily/Weekly/Monthly/RCT Efficiency) + KCC
   Renewal & Rollover -- six region-wise reports from Head Office, each
   published independently (Alok: "kabhi koi hogi kabhi sari", so these
   are NOT bundled into one upload the way NPA+extras sometimes are).
   All six share one generic sortable/filterable region-table renderer;
   the Hathras row is always highlighted since that's the user's own
   region across every one of these reports. ---------- */
function isHathrasRegion(name){ return String(name||'').trim().toUpperCase()==='HATHRAS'; }
/* toDate() (shared app-wide) only accepts DD-MM-YYYY or Excel serials --
   these 6 datasets store asOnDate as plain ISO (YYYY-MM-DD), so this
   local helper covers that one extra shape without touching toDate()
   itself and risking every other caller across the app. */
...
function titleCaseRegion(s){
  return String(s||'').toLowerCase().replace(/(^|[\s-])[a-z]/g, m=>m.toUpperCase());
}
function fmtRTCr(v){ return (v===null||v===undefined||isNaN(v)) ? '—' : Number(v).toLocaleString('en-IN',{minimumFractionDigits:2,maximumFractionDigits:2}); }
function fmtRTInt(v){ return (v===null||v===undefined||isNaN(v)) ? '—' : Math.round(v).toLocaleString('en-IN'); }
function fmtRTPct(v){ return (v===null||v===undefined||isNaN(v)) ? '—' : Number(v).toFixed(2)+'%'; }
function rtPctClass(v, ref){
  if(v===null||v===undefined||isNaN(v)) return '';
  if(ref!==undefined && ref!==null && !isNaN(ref)){
    if(v >= ref) return 'rt-good';
    if(v >= ref*0.85) return 'rt-mid';
    return 'rt-bad';
  }
  if(v>=70) return 'rt-good';
  if(v>=45) return 'rt-mid';
  return 'rt-bad';
}

const RT_STATE = {}; // per-table-id: {sort:{key,dir}, filters:{}}
function rtGetState(tableId, defaultKey, defaultDir){
  if(!RT_STATE[tableId]) RT_STATE[tableId] = {sort:{key:defaultKey, dir:defaultDir||'asc'}, filters:{}};
  return RT_STATE[tableId];
}
const RT_RERENDER = {}; // tableId -> rerender callback, kept current across renders (no listener pileup)
const RT_LISTENERS_WIRED = new Set();
function rtOnRerender(tableId, fn){
  RT_RERENDER[tableId] = fn;
  if(!RT_LISTENERS_WIRED.has(tableId)){
    RT_LISTENERS_WIRED.add(tableId);
    document.addEventListener('rt-rerender-'+tableId, ()=>{ const cb = RT_RERENDER[tableId]; if(cb) cb(); });
  }
}

function renderRegionTable(tableId, columns, rows, totalsRow, opts){
  opts = opts || {};
  const state = rtGetState(tableId, opts.defaultKey||'region', opts.defaultDir||'asc');
  // grouped header row (colspan runs of equal .group)
  let groupHtml = '';
  { let i=0; const parts=[];
    while(i<columns.length){
      const g = columns[i].group;
      let j=i; while(j<columns.length && columns[j].group===g) j++;
      const span = j-i;
      if(g==null) parts.push(`<th rowspan="2" class="${columns[i].align==='left'?'rt-left':''}">${esc(columns[i].label)}</th>`);
      else parts.push(`<th colspan="${span}">${esc(g)}</th>`);
      i = (g==null) ? i+1 : j;
    }
    groupHtml = '<tr class="rt-grp-row">'+parts.join('')+'</tr>';
  }
  const subCols = columns.filter(c=>c.group!=null);
  const subHtml = '<tr class="rt-sub-row">'+subCols.map(c=>
    `<th class="rt-sortable" data-key="${c.key}" tabindex="0" role="button" aria-sort="none" onclick="rtSortBy('${tableId}','${c.key}')">${esc(c.label)}`
    +`<span class="rt-hicons"><svg class="rt-sort-ic" viewBox="0 0 10 10"><path d="M5 1l3 4H2z" fill="currentColor"/></svg>`
    +`<svg class="rt-filter-ic" viewBox="0 0 10 10" onclick="event.stopPropagation();rtToggleFilter('${tableId}','${c.key}')"><path d="M1 1h8L6 5v3L4 9V5z" fill="currentColor"/></svg></span></th>`
  ).join('')+'</tr>';
  const filterHtml = '<tr class="rt-filter-row" id="rtFilterRow-'+tableId+'" style="display:'+(Object.values(state.filters).some(v=>v)?'':'none')+'">'
    + '<td colspan="'+(columns.filter(c=>c.group==null).length)+'"></td>'
    + subCols.map(c=>`<td><input type="text" class="rt-filter-input" placeholder="Filter" value="${esc(state.filters[c.key]||'')}" oninput="rtSetFilter('${tableId}','${c.key}',this.value)"></td>`).join('')
    + '</tr>';

  let list = rows.slice();
  const activeFilters = Object.entries(state.filters).filter(([k,v])=>v);
  if(activeFilters.length){
    list = list.filter(r => activeFilters.every(([key,val])=>{
      const col = columns.find(c=>c.key===key);
      const cell = col && col.fmt ? col.fmt(r[key], r) : r[key];
  ...
  const bodyRows = list.map(r=>{
    const hathras = isHathrasRegion(r.region);
    return `<tr class="${hathras?'rt-hathras-row':''}">` + columns.map(c=>{
      const raw = r[c.key];
      const val = c.key==='region' ? (hathras?`<span class="rt-pin"><svg viewBox="0 0 20 20" fill="none"><path d="M10 1a5 5 0 00-5 5c0 4 5 12 5 12s5-8 5-12a5 5 0 00-5-5z" fill="currentColor"/></svg>${esc(titleCaseRegion(raw))}</span>` : esc(titleCaseRegion(raw)))
        : (c.fmt ? c.fmt(raw, r) : esc(raw==null?'—':raw));
      const cls = c.align==='left' ? 'rt-left' : (c.pctClass ? c.pctClass(raw,r) : '');
      return `<td class="${cls}">${val}</td>`;
    }).join('') + '</tr>';
  }).join('');

  const totalHtml = totalsRow ? `<tr class="rt-total-row"><td class="rt-left">${esc(totalsRow.label||'Grand Total')}</td>` + columns.slice(1).map(c=>{
    const raw = totalsRow[c.key];
    const val = c.fmt ? c.fmt(raw, totalsRow) : esc(raw==null?'—':raw);
    return `<td class="${c.align==='left'?'rt-left':''}">${val}</td>`;
  }).join('') + '</tr>' : '';

  return `<div class="rt-wrap"><table class="rt-table" id="${tableId}">
    <thead>${groupHtml}${subHtml}${filterHtml}</thead>
    <tbody>${bodyRows||`<tr><td colspan="${columns.length}" style="text-align:center;color:var(--ink-mute);padding:22px">No rows match this filter</td></tr>`}</tbody>
    ${totalHtml ? `<tfoot>${totalHtml}</tfoot>` : ''}
  </table></div>`;
}
window.rtSortBy = function(tableId, key){
  const state = rtGetState(tableId);
  state.sort = nextSort(state.sort, key);
  document.dispatchEvent(new CustomEvent('rt-rerender-'+tableId));
};
window.rtToggleFilter = function(tableId, key){
  const row = document.getElementById('rtFilterRow-'+tableId);
  if(row) row.style.display = row.style.display==='none' ? '' : 'none';
};
window.rtSetFilter = function(tableId, key, val){
  const state = rtGetState(tableId);
  state.filters[key] = val;
  document.dispatchEvent(new CustomEvent('rt-rerender-'+tableId));
};

/* -------- Uploads for the 6 PNPA/KCC reports --------
   Each one is staged and published entirely independently of the others
   (Alok: "ye jaruri nahi hai ki daily sari files ek sath upload ho payenge
   kabhi koi hogi kabhi sari") -- uploading one never requires or disturbs
   any of the other five, mirroring the existing Bank/PNPA/KCC-Overdue
   upload pattern exactly. */
let __pendingPnpaDailyData = null, __pendingPnpaWeeklyData = null, __pendingPnpaMonthlyData = null;
let __pendingRctEfficiencyData = null, __pendingKccRenewalData = null, __pendingKccRolloverData = null;

function handleRegionReportUpload(opts){
  // opts: {inputId, statusId, statusLabelId, parseFn, setPending, refreshView}
  const file = document.getElementById(opts.inputId).files[0];
  if(!file) return;
  const statusEl = document.getElementById(opts.statusId);
  const reader = new FileReader();
  reader.onload = async () => {
    try{
      const parsed = await opts.parseFn(reader.result);
      opts.setPending(parsed);
      const label = document.getElementById(opts.statusLabelId);
      if(label) label.textContent = `${parsed.regions.length} regions loaded (${file.name})`;
      const warnLine = parsed.warning ? `<div class="upload-status err" style="margin-top:6px">⚠ ${esc(parsed.warning)}</div>` : '';
      const displayDate = parsed.asOnDate ? esc(fmtDate(isoToDate(parsed.asOnDate))) : 'unknown date';
      statusEl.innerHTML = `<div class="upload-status ok">✔ Parsed ${parsed.regions.length} region(s), as on ${displayDate}. Goes live the next time you hit Publish.</div>${warnLine}`;
      clearStalePublishStatus();
      const publishBtn = document.getElementById('publishBtn');
      if(publishBtn) publishBtn.disabled = false;
      if(opts.refreshView) opts.refreshView();
    } catch(err){
      statusEl.innerHTML = `<div class="upload-status err">⚠ Could not read this file: ${esc(err.message||err)}</div>`;
    }
  };
...
function handlePnpaDailyUpload(){
  handleRegionReportUpload({ inputId:'pnpaDailyFileInput', statusId:'pnpaDailyUploadStatus', statusLabelId:'pnpaDailyStatusLabel',
    parseFn: parsePnpaDailyPdf, setPending: (d)=>{ __pendingPnpaDailyData = d; PNPA_REP_DATA.daily = d; },
    refreshView: ()=>{ if(document.querySelector('.view.active')?.dataset.view==='pnparep' && pnpaRepView==='daily') renderPnpaReportsBody(); } });
}
function handlePnpaWeeklyUpload(){
  handleRegionReportUpload({ inputId:'pnpaWeeklyFileInput', statusId:'pnpaWeeklyUploadStatus', statusLabelId:'pnpaWeeklyStatusLabel',
    parseFn: parsePnpaWeeklyPdf, setPending: (d)=>{ __pendingPnpaWeeklyData = d; PNPA_REP_DATA.weekly = d; },
    refreshView: ()=>{ if(document.querySelector('.view.active')?.dataset.view==='pnparep' && pnpaRepView==='weekly') renderPnpaReportsBody(); } });
}
function handlePnpaMonthlyUpload(){
  handleRegionReportUpload({ inputId:'pnpaMonthlyFileInput', statusId:'pnpaMonthlyUploadStatus', statusLabelId:'pnpaMonthlyStatusLabel',
    parseFn: parsePnpaMonthlyPdf, setPending: (d)=>{ __pendingPnpaMonthlyData = d; PNPA_REP_DATA.monthly = d; },
    refreshView: ()=>{ if(document.querySelector('.view.active')?.dataset.view==='pnparep' && pnpaRepView==='monthly') renderPnpaReportsBody(); } });
}
function handleRctEfficiencyUpload(){
  handleRegionReportUpload({ inputId:'rctEfficiencyFileInput', statusId:'rctEfficiencyUploadStatus', statusLabelId:'rctEfficiencyStatusLabel',
    parseFn: parseRctEfficiencyPdf, setPending: (d)=>{ __pendingRctEfficiencyData = d; PNPA_REP_DATA.rct = d; },
    refreshView: ()=>{ if(document.querySelector('.view.active')?.dataset.view==='pnparep' && pnpaRepView==='rct') renderPnpaReportsBody(); } });
}
function handleKccRenewalUpload(){
  handleRegionReportUpload({ inputId:'kccRenewalFileInput', statusId:'kccRenewalUploadStatus', statusLabelId:'kccRenewalStatusLabel',
    parseFn: parseKccRenewalPdf, setPending: (d)=>{ __pendingKccRenewalData = d; KCC_RENEWAL_DATA = d; },
    refreshView: ()=>{ if(document.querySelector('.view.active')?.dataset.view==='kccren' && kccRenView==='renewal') renderKccRenewalBody(); } });
}
function handleKccRolloverUpload(){
  handleRegionReportUpload({ inputId:'kccRolloverFileInput', statusId:'kccRolloverUploadStatus', statusLabelId:'kccRolloverStatusLabel',
    parseFn: parseKccRolloverPdf, setPending: (d)=>{ __pendingKccRolloverData = d; KCC_ROLLOVER_DATA = d; },
    refreshView: ()=>{ if(document.querySelector('.view.active')?.dataset.view==='kccren' && kccRenView==='rollover') renderKccRenewalBody(); } });
}

/* -------- PNPA Reports -------- */
let PNPA_REP_DATA = { daily:null, weekly:null, monthly:null, rct:null };
let pnpaRepView = 'daily';
function setPnpaRepView(v){ pnpaRepView = v; renderPnpaReportsBody(); }
window.setPnpaRepView = setPnpaRepView;

function renderPnpaReports(){
  const el = document.getElementById('pnpaRepArea');
  if(!el) return;
  renderPnpaReportsBody();
}
function pnpaRepFile(v){ return {daily:'pnpa-daily', weekly:'pnpa-weekly', monthly:'pnpa-monthly', rct:'rct-efficiency'}[v]; }
function renderPnpaReportsBody(){
  const el = document.getElementById('pnpaRepArea');
  if(!el) return;
  const v = pnpaRepView;
  const tabs = `<div class="bank-tab-row">` + ['daily','weekly','monthly','rct'].map(k=>
    `<button type="button" class="bank-tab-btn${v===k?' active':''}" onclick="setPnpaRepView('${k}')">${{daily:'Daily',weekly:'Weekly',monthly:'Monthly',rct:'RCT Efficiency'}[k]}</button>`
  ).join('') + `</div>`;
  if(!PNPA_REP_DATA[v]){
    el.innerHTML = tabs + `<div class="empty-state"><div class="data-loading-spinner" aria-hidden="true" style="position:static;border-color:rgba(58,123,255,.25);border-top-color:var(--accent)"></div><p style="margin-top:14px">Loading…</p></div>`;
    fetchJson('data/'+pnpaRepFile(v)+'.json?t=' + Date.now())
      .then(d => { PNPA_REP_DATA[v] = d; if(pnpaRepView===v) renderPnpaReportsBody(); })
      .catch(() => { if(pnpaRepView===v) el.innerHTML = tabs + `<div class="empty-state"><h2>Could not load this report</h2><p>Check your internet connection, then tap Refresh.</p></div>`; });
    return;
  }
  const d = PNPA_REP_DATA[v];
  const tableId = 'pnpaRepTbl-'+v;
  let html;
  if(v==='daily' || v==='weekly'){
    const cols = [
      {key:'region', label:'Region', group:null, align:'left'},
      {key:'branches', label:'Br.', group:null, align:'left'},
      {key:'totalAcc', label:'A/c', group:'Total', fmt:fmtRTInt},
      {key:'totalAmt', label:'Amt', group:'Total', fmt:fmtRTCr},
      {key:'kccAcc', label:'A/c', group:'KCC', fmt:fmtRTInt},
      {key:'kccAmt', label:'Amt', group:'KCC', fmt:fmtRTCr},
      {key:'tlAcc', label:'A/c', group:'TL', fmt:fmtRTInt},
      {key:'tlAmt', label:'Amt', group:'TL', fmt:fmtRTCr},
      {key:'ccodAcc', label:'A/c', group:'CC/OD', fmt:fmtRTInt},
      {key:'ccodAmt', label:'Amt', group:'CC/OD', fmt:fmtRTCr},
      {key:'nonfinAcc', label:'A/c', group:'Non-Financial', fmt:fmtRTInt},
      {key:'nonfinAmt', label:'Amt', group:'Non-Financial', fmt:fmtRTCr},
      {key:'custAcc', label:'A/c', group:'Custlevel', fmt:fmtRTInt},
      {key:'custAmt', label:'Amt', group:'Custlevel', fmt:fmtRTCr},
    ];
    html = renderRegionTable(tableId, cols, d.regions, d.grandTotal, {defaultKey:'region', defaultDir:'asc'});
  } else if(v==='monthly'){
    const cols = [
      {key:'region', label:'Region', group:null, align:'left'},
      {key:'branches', label:'Br.', group:null, align:'left'},
      {key:'pnpaAug', label:'Aug-26', group:'Total PNPA', fmt:fmtRTCr},
      {key:'remainingPnpa', label:'Remain.', group:'Total PNPA', fmt:fmtRTCr},
      {key:'turnedToNpa', label:'Turned', group:'Total PNPA', fmt:fmtRTCr},
      {key:'reduction', label:'Red.', group:'Total PNPA', fmt:fmtRTCr},
      {key:'pctRed', label:'%Red', group:'Total PNPA', fmt:fmtRTPct, pctClass:(v)=>rtPctClass(v,69)},
      {key:'kccAug', label:'Aug-26', group:'Out of KCC', fmt:fmtRTCr},
      {key:'kccRemaining', label:'Remain.', group:'Out of KCC', fmt:fmtRTCr},
      {key:'kccTurned', label:'Turned', group:'Out of KCC', fmt:fmtRTCr},
      {key:'kccReduction', label:'Red.', group:'Out of KCC', fmt:fmtRTCr},
      {key:'pctNpaKcc', label:'%NPA', group:'Out of KCC', fmt:fmtRTPct, pctClass:(v)=>rtPctClass(v,35)},
      {key:'pctRedKcc', label:'%Red', group:'Out of KCC', fmt:fmtRTPct, pctClass:(v)=>rtPctClass(v,34)},
      {key:'tlAug', label:'Aug-26', group:'Out of Term Loan', fmt:fmtRTCr},
      {key:'tlRemaining', label:'Remain.', group:'Out of Term Loan', fmt:fmtRTCr},
      {key:'pctRedTl', label:'%Red', group:'Out of Term Loan', fmt:fmtRTPct, pctClass:(v)=>rtPctClass(v,29)},
      {key:'ccodAug', label:'Aug-26', group:'Out of CC/OD', fmt:fmtRTCr},
      {key:'ccodRemaining', label:'Remain.', group:'Out of CC/OD', fmt:fmtRTCr},
      {key:'pctRedCcod', label:'%Red', group:'Out of CC/OD', fmt:fmtRTPct, pctClass:(v)=>rtPctClass(v,84)},
      {key:'nonfinAug', label:'Aug-26', group:'Non-Financial', fmt:fmtRTCr},
      {key:'nonfinRemaining', label:'Remain.', group:'Non-Financial', fmt:fmtRTCr},
      {key:'pctRedNonfin', label:'%Red', group:'Non-Financial', fmt:fmtRTPct, pctClass:(v)=>rtPctClass(v,91)},
    ];
    html = renderRegionTable(tableId, cols, d.regions, {...d.grandTotal, label:'UPGB Total'}, {defaultKey:'region', defaultDir:'asc'});
  } else { // rct
    const cols = [
      {key:'region', label:'Region', group:null, align:'left'},
      {key:'branches', label:'Br.', group:null, align:'left'},
      {key:'rolloverMar', label:'31-Mar', group:'Rollover Efficiency', fmt:fmtRTPct},
      {key:'rolloverJul', label:'31-Jul', group:'Rollover Efficiency', fmt:fmtRTPct},
      {key:'rolloverAug', label:'17-Aug', group:'Rollover Efficiency', fmt:fmtRTPct, pctClass:(v)=>rtPctClass(v,73.44)},
      {key:'collectionMar', label:'31-Mar', group:'Collection Efficiency', fmt:fmtRTPct},
      {key:'collectionJul', label:'31-Jul', group:'Collection Efficiency', fmt:fmtRTPct},
      {key:'collectionAug', label:'17-Aug', group:'Collection Efficiency', fmt:fmtRTPct, pctClass:(v)=>rtPctClass(v,86.07)},
      {key:'turnoverMar', label:'31-Mar', group:'Turnover Efficiency', fmt:fmtRTPct},
      {key:'turnoverJul', label:'31-Jul', group:'Turnover Efficiency', fmt:fmtRTPct},
      {key:'turnoverAug', label:'17-Aug', group:'Turnover Efficiency', fmt:fmtRTPct, pctClass:(v)=>rtPctClass(v,85.67)},
    ];
    html = renderRegionTable(tableId, cols, d.regions, {...d.grandTotal, label:'UPGB Total'}, {defaultKey:'region', defaultDir:'asc'});
  }
  const asOnLabel = v==='daily' ? `Daily, as on ${fmtDate(isoToDate(d.asOnDate))}`
    : v==='weekly' ? `Weekly, ${d.weekLabel||''}`
    : v==='monthly' ? `Monthly, ${d.monthLabel||''} (as on ${fmtDate(isoToDate(d.asOnDate))})`
    : `Rollover / Collection / Turnover, as on ${fmtDate(isoToDate(d.asOnDate))}`;
  document.querySelectorAll('.pnpaRep-asOn').forEach(e=>e.textContent = asOnLabel);
  el.innerHTML = tabs + html;
  rtOnRerender(tableId, ()=>{ if(pnpaRepView===v) renderPnpaReportsBody(); });
}

/* -------- KCC Renewal & Rollover -------- */
let KCC_RENEWAL_DATA = null, KCC_ROLLOVER_DATA = null;
let kccRenView = 'renewal';
function setKccRenView(v){ kccRenView = v; renderKccRenewalBody(); }
window.setKccRenView = setKccRenView;

function renderKccRenewal(){
  const el = document.getElementById('kccRenArea');
  if(!el) return;
  renderKccRenewalBody();
}
function renderKccRenewalBody(){
  const el = document.getElementById('kccRenArea');
  if(!el) return;
  const v = kccRenView;
  const tabs = `<div class="bank-tab-row">
    <button type="button" class="bank-tab-btn${v==='renewal'?' active':''}" onclick="setKccRenView('renewal')">Renewal</button>
    <button type="button" class="bank-tab-btn${v==='rollover'?' active':''}" onclick="setKccRenView('rollover')">Rollover</button>
  </div>`;
  const need = v==='renewal' ? !KCC_RENEWAL_DATA : !KCC_ROLLOVER_DATA;
  if(need){
    el.innerHTML = tabs + `<div class="empty-state"><div class="data-loading-spinner" aria-hidden="true" style="position:static;border-color:rgba(58,123,255,.25);border-top-color:var(--accent)"></div><p style="margin-top:14px">Loading…</p></div>`;
    const file = v==='renewal' ? 'kcc-renewal' : 'kcc-rollover';
    fetchJson('data/'+file+'.json?t=' + Date.now())
      .then(d => { if(v==='renewal') KCC_RENEWAL_DATA = d; else KCC_ROLLOVER_DATA = d; if(kccRenView===v) renderKccRenewalBody(); })
      .catch(() => { if(kccRenView===v) el.innerHTML = tabs + `<div class="empty-state"><h2>Could not load this report</h2><p>Check your internet connection, then tap Refresh.</p></div>`; });
    return;
  }
  let html, asOnLabel;
  if(v==='renewal'){
    const d = KCC_RENEWAL_DATA;
    const cols = [
      {key:'region', label:'Region', group:null, align:'left'},
      {key:'branches', label:'Br.', group:null, align:'left'},
      {key:'dueAcc', label:'A/c', group:'Total KCC & Tatkal Due', fmt:fmtRTInt},
      {key:'dueAmt', label:'Amt', group:'Total KCC & Tatkal Due', fmt:fmtRTCr},
      {key:'renewedAcc', label:'A/c', group:'Renewed', fmt:fmtRTInt},
      {key:'renewedAmt', label:'Amt', group:'Renewed', fmt:fmtRTCr},
      {key:'pctRenewal', label:'%Renewal', group:null, fmt:fmtRTPct, pctClass:(v)=>rtPctClass(v,54)},
      {key:'turnedAcc', label:'A/c', group:'Turned Into NPA', fmt:fmtRTInt},
      {key:'turnedAmt', label:'Amt', group:'Turned Into NPA', fmt:fmtRTCr},
      {key:'pendingAcc', label:'A/c', group:'Pending Renewal', fmt:fmtRTInt},
      {key:'pendingAmt', label:'Amt', group:'Pending Renewal', fmt:fmtRTCr},
      {key:'lastDayAcc', label:'A/c', group:'Last Day Progress', fmt:fmtRTInt},
      {key:'lastDayAmt', label:'Amt', group:'Last Day Progress', fmt:fmtRTCr},
      {key:'rank', label:'Rank', group:null, fmt:(v)=>v==null?'—':`<span class="rt-rank">${Math.round(v)}</span>`},
    ];
    html = renderRegionTable('kccRenTbl', cols, d.regions, {...d.grandTotal, label:'UPGB Total'}, {defaultKey:'rank', defaultDir:'asc'});
    asOnLabel = `Renewal, ${d.fyLabel||''} till ${fmtDate(isoToDate(d.asOnDate))}`;
  } else {
    const d = KCC_ROLLOVER_DATA;
    const cols = [
      {key:'region', label:'Region', group:null, align:'left'},
      {key:'branches', label:'Br.', group:null, align:'left'},
      {key:'stdAcc', label:'A/c', group:'KCC STD A/c (31-Mar)', fmt:fmtRTInt},
      {key:'stdAmt', label:'Amt', group:'KCC STD A/c (31-Mar)', fmt:fmtRTCr},
      {key:'rolloverTotalAcc', label:'A/c', group:'Rollover Total', fmt:fmtRTInt},
      {key:'rolloverTotalAmt', label:'Amt', group:'Rollover Total', fmt:fmtRTCr},
      {key:'rolloverMar27Acc', label:'A/c', group:'Rollover in Mar-27 PNPA', fmt:fmtRTInt},
      {key:'rolloverMar27Amt', label:'Amt', group:'Rollover in Mar-27 PNPA', fmt:fmtRTCr},
      {key:'pctRollover', label:'%Rollover', group:null, fmt:fmtRTPct, pctClass:(v)=>rtPctClass(v,30)},
      {key:'prevTotalAcc', label:'A/c', group:'Prev. Day Total', fmt:fmtRTInt},
      {key:'prevTotalAmt', label:'Amt', group:'Prev. Day Total', fmt:fmtRTCr},
      {key:'prevMar27Acc', label:'A/c', group:'Prev. in Mar-27 PNPA', fmt:fmtRTInt},
      {key:'prevMar27Amt', label:'Amt', group:'Prev. in Mar-27 PNPA', fmt:fmtRTCr},
    ];
    html = renderRegionTable('kccRollTbl', cols, d.regions, {...d.grandTotal, label:'UPGB Total'}, {defaultKey:'rank', defaultDir:'asc'});
    asOnLabel = `Rollover, as on ${fmtDate(isoToDate(d.asOnDate))}`;
  }
  document.querySelectorAll('.kccRen-asOn').forEach(e=>e.textContent = asOnLabel);
  el.innerHTML = tabs + html;
  const tableId = v==='renewal' ? 'kccRenTbl' : 'kccRollTbl';
  rtOnRerender(tableId, ()=>{ if(kccRenView===v) renderKccRenewalBody(); });
}
...
function switchView(view){
  ...
  if(view==='pnparep') renderPnpaReports();
  if(view==='kccren') renderKccRenewal();
...
(function wireChrome(){
  ...
  on('pnpaDailyUploadDrop','click',()=>document.getElementById('pnpaDailyFileInput').click());
  on('pnpaDailyFileInput','change',()=>handlePnpaDailyUpload());
  on('pnpaWeeklyUploadDrop','click',()=>document.getElementById('pnpaWeeklyFileInput').click());
  on('pnpaWeeklyFileInput','change',()=>handlePnpaWeeklyUpload());
  on('pnpaMonthlyUploadDrop','click',()=>document.getElementById('pnpaMonthlyFileInput').click());
  on('pnpaMonthlyFileInput','change',()=>handlePnpaMonthlyUpload());
  on('rctEfficiencyUploadDrop','click',()=>document.getElementById('rctEfficiencyFileInput').click());
  on('rctEfficiencyFileInput','change',()=>handleRctEfficiencyUpload());
  on('kccRenewalUploadDrop','click',()=>document.getElementById('kccRenewalFileInput').click());
  on('kccRenewalFileInput','change',()=>handleKccRenewalUpload());
  on('kccRolloverUploadDrop','click',()=>document.getElementById('kccRolloverFileInput').click());
  on('kccRolloverFileInput','change',()=>handleKccRolloverUpload());
