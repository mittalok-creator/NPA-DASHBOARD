...
### New: Customer Master address now reaches KCC Overdue/PNPA too, via a Customer-ID-keyed map independent of the NPA book (2026-09-25, same day)

Alok, after confirming the leading-zero Customer Master fix (below) worked for the NPA book itself: "fir kcc overdue main kyun nahi aa raha" (then why isn't it showing in KCC Overdue) -- Recovery Dashboard's KCC Overdue tab (and, it turns out, its PNPA Slippage tab too) always showed "—" in their own Address column, no matter how correctly Customer Master was uploaded and published.

Root cause, confirmed by direct code read: Recovery Dashboard's `addressForAcctNo()` only ever resolves an address by matching an Account No. against `DATA.npa.rows` (the NPA book). But KCC Overdue and PNPA are genuinely **disjoint account universes** from the NPA book -- an account that's overdue-but-still-standard (KCC Overdue) or that only just slipped this week/month (PNPA) has, by definition, zero overlap with the already-NPA accounts making up `DATA.npa.rows` (confirmed earlier this session: zero shared account numbers). So no matter how well Customer Master's addresses were merged into the NPA book, that merge could never reach these two datasets' own accounts -- there was no bug in the merge itself, the address source these two views relied on simply didn't (and structurally couldn't) cover them.

**Fix**: Customer Master's full, ~75k-customer address map (Customer ID -> Address, not just the subset already merged into the current NPA book) is now kept as its own persistent field, `DATA.customerAddressMap`, rebuilt in full every time Customer Master is (re-)uploaded in the main app and included in the publish payload -- same "own slow-moving schedule, part of Publish" treatment as `branchAdvances`/`specialNotes`/etc. Both `parseKccOverdueRows()` and `parsePnpaRows()` (shared by Daily/Weekly/Monthly PNPA) now also capture each row's own Customer ID, when the source file has that column (it does, for both -- confirmed against real files), so Recovery Dashboard's `addressForAcctNo()` (via `kccovAddressFor()`/`pnpaAddressFor()`, both now taking an optional Customer ID) can fall back to this map by Customer ID whenever the Account No. lookup against the NPA book comes up empty. Purely additive -- no existing row shape/index changes for anything already reading these files by position, since the new Customer ID field is appended at the end of each row array.

Verified with a Node reproduction: a synthetic KCC Overdue row for an account genuinely absent from the NPA book, whose Customer ID IS present in `customerAddressMap` -- the old call site (no Customer ID passed) resolves to blank, confirming the bug; the fixed call site (Customer ID passed through) resolves the real address correctly. `node --check js/app.js` clean on both repos.

Alok will need to re-upload Customer Master once more (or simply Publish again if it's already uploaded this session) once this ships, so `customerAddressMap` actually gets published for the first time -- KCC Overdue and PNPA addresses will only start resolving from that publish onward, not retroactively from an earlier one.

### Fix: Customer Master merge silently failed on leading-zero/scientific-notation Customer ID mismatches -- root cause of "Publish succeeds but addresses don't update" (2026-09-25, same day)

Alok uploaded a fresh Customer Master file (74,815 records) and, per his own screenshot, clicked "Confirm & Publish" -- the "Published" success message appeared with a real commit, but the live site didn't show the updated addresses. Since Customer Master is now the *sole* address source app-wide (the separate Address List feature was removed earlier today), any bug in its own merge silently breaks addresses everywhere at once -- Dashboard, KCC Overdue, Branch Split, Recovery Dashboard.

Investigated the actual merge pipeline directly rather than assuming a hosting/caching issue: `buildCustomerMasterMap()` keyed its map by the master file's raw `cellStr(row, iCust)`, and both `mergeCustomerDetails()` and `carryForwardMapFromCurrentData()` looked customers up by the NPA row's raw `String(r[C.CUST_ID])` -- **neither side ever normalized the Customer ID**, unlike the two read-only Branch Split bridge functions (`UPGB_getAddressList`/`...ByCustomer`, `js/app.js:142-148`) which already call `normId()` for exactly this reason. A Customer ID stored as text with a leading zero in the master file (a common CIF format) will never match the same customer's ID in the daily NPA book once Excel/XLSX parsing has silently dropped that leading zero on the numeric side (or turned a long ID into scientific notation) -- so any customer hitting this mismatch got no address at all, no matter how many times Customer Master was re-uploaded and published. This is the exact same failure class `normId()` was built to fix for the old Address List feature months ago -- it just never got carried over into Customer Master's own matching pipeline.

**Fix**: `buildCustomerMasterMap()` now keys its map by `normId(cellStr(row, iCust))`; `mergeCustomerDetails()` and `carryForwardMapFromCurrentData()` now both look up/key by `normId(r[C.CUST_ID])` instead of a plain `String(...)`. No change to what's actually matched conceptually (still 100% Customer-ID-based) -- purely a normalization fix on both sides of the same comparison, mirroring the bridge functions' own existing `normId()` calls.

Verified with a Node reproduction (not the full browser flow, since Settings/Update Data sits behind Admin login): built the exact old (buggy) and new (fixed) versions of `buildCustomerMasterMap`/`mergeCustomerDetails` side by side against a master file with a leading-zero-text Customer ID ("04521178") and a matching NPA row whose own Customer ID came through as a plain number (4521178, leading zero already lost) -- the old code left that customer's address blank, the new code correctly resolves it, and a normal non-leading-zero customer ID still matches exactly as before in both versions (no regression). `node --check js/app.js` clean.

Alok will need to re-upload his Customer Master file (the same one is fine) and Publish once more once this ships -- accounts that silently failed to match under the old code will pick up their address correctly this time.

### New: Weekly PNPA and Monthly PNPA uploads, powering Recovery Dashboard's "This Week"/"This Month" tabs from their own dedicated files (2026-09-25, same day)

Alok: "recovery dashboard main jo pnpa k liye today, weekly and monthly banaye hai uske data bhi upload hoga daily basis par wo bhi add karo daily pnpa to already hai weekly and monthly bhi add karo upload hone k liye treat and fetch karne ka method daily wale ki tarah hi rahega" (add Weekly and Monthly PNPA uploads alongside the existing Daily one -- same parse/fetch method as Daily). Confirmed directly (AskUserQuestion): the Weekly/Monthly files Head Office provides are already scoped to their own period (a genuine "this week's slippage" / "this month's slippage" report), not a whole-book snapshot needing a date-window filter applied downstream -- so Recovery Dashboard's Week/Month tabs now show these files' own rows as-is (branch-filtered only), replacing the previous approach of deriving "This Week"/"This Month" by day-count-bucketing off the single Daily PNPA file.

**`js/app.js`**: two new upload handlers, `handleWeeklyPnpaUpload`/`handleMonthlyPnpaUpload`, mirroring `handlePnpaUpload` exactly (same `parsePnpaRows()` parser, same Hathras/zero-balance filtering) but writing into their own state (`PNPA_WEEKLY_DATA`/`__pendingPnpaWeeklyData`, and the Monthly equivalents) and their own status labels -- deliberately NOT refactored into a shared function with the existing, already-shipped `handlePnpaUpload`, to avoid any risk of changing Daily PNPA's own working behavior. Publish flow (`confirmPublish`) gained matching `weeklyPnpaLabel`/`monthlyPnpaLabel` construction, review-panel line items, and `extraFiles` entries publishing `data/pnpa-weekly.json`/`data/pnpa-monthly.json` alongside the existing `data/pnpa.json`/`data/kcc-overdue.json` -- `js/publish.js` needed no logic changes at all (its `extraFiles` handling was already fully generic over path/content/label, confirmed by reading it directly rather than assuming). `pendingUnpublishedLabel()` (the unsaved-changes warning shown on Refresh/tab-close) extended to mention either upload if pending.

**`index.html`**: two new cards in the Update Data hub's "Daily Files" category (now 4 files: Daily/Weekly/Monthly PNPA + KCC Overdue) and two new full-screen panels, built on the exact same structure as Daily PNPA's own panel -- no template-download button, matching Daily PNPA's own precedent (there is no "blank template" for this file type).

**Recovery Dashboard** (`dashboard.alokmittal.net`, separate repo): fetches the two new files cross-origin, same pattern as the existing Daily PNPA/KCC Overdue fetches. New `pnpaAggregateToday()` (unchanged day-count bucketing off Daily PNPA, for "Today" only) replaces the old `pnpaAggregateSlippage()`, and new `pnpaAggregatePeriod()` (branch-filter only, no date bucketing) computes "This Week"/"This Month" straight from their own files. A missing/not-yet-published Weekly or Monthly file doesn't block the page -- that one tab shows a clear "not uploaded yet" empty state while Today keeps working normally, tracked via `__pnpaWeeklyFailed`/`__pnpaMonthlyFailed` so a failed fetch is never silently retried on every render.

Verified end-to-end against the real production PIN login (not simulated): uploaded a synthetic 2-account CSV through both new Update Data panels, confirmed the panel status/tile status/Publish-review-panel entries ("Weekly PNPA 2 accounts... Included", "Monthly PNPA 2 accounts... Included") all populated correctly -- stopped short of actually clicking "Confirm & Publish" so no test data went live. Separately verified Recovery Dashboard's own real production PIN login: with `data/pnpa-weekly.json`/`data/pnpa-monthly.json` not yet published (expected, since nothing's been uploaded for real yet), the Week/Month tabs correctly show "No Weekly/Monthly PNPA file uploaded yet." rather than crashing or hanging, while the Today tab (real Daily PNPA data) kept working unaffected. `node --check` on `js/app.js`/`js/publish.js` in both repos, all clean. Zero JS errors in either verification pass (only the two expected 404s for the not-yet-published files).

### Removed: the separate "Address List" upload -- Customer Master is now the sole, app-wide address source (2026-09-25, same day)

Alok: "Address list hata kar customer master hi rahne do and agar ek baar customer master push ho gaya to fir wo har app main har tab main har utility jahan address chahiye wahi se link hoga without any error" (remove Address List, keep only Customer Master -- once it's published, address should link from there alone, everywhere, without error).

The two upload panels had overlapping purpose and had already gone through several rounds of leading-zero/Customer-ID matching fixes (see the 2026-09-23 entries below) -- consolidating down to one, always-correct source removes that whole class of "address list didn't match" bug outright rather than fixing it again.

**`js/app.js`**: the "Address List" upload panel, its parser (`buildAddressListMap`), its upload handler (`handleAddressListUpload`), its template download (`downloadAddressListTemplate`), its retroactive fallback pass (`applyAddressListFallback`), its fallback branch inside `mergeCustomerDetails()`, its two published fields (`DATA.addressList`/`DATA.addressListByCustomer`) and their `renormalizeIdMap()` helper, and all of its wiring/tile-sync/publish-payload references were removed entirely. `window.UPGB_getAddressList()`/`window.UPGB_getAddressListByCustomer()` -- the same-origin bridge `tools/branch-split.html` reads for its own address enrichment -- are **not** removed, only repointed: instead of returning the old separate list, they now build an Account-No./Customer-ID -> Address map fresh off `DATA.npa.rows`' own already-merged `C.ADDR` field each call. Since Customer Master's own merge (`mergeCustomerDetails()`) already writes into `C.ADDR` and `carryForwardMapFromCurrentData()` already carries a customer's address into every later daily upload even without re-uploading Customer Master, this means Branch Split (and anything else reading the bridge) now gets exactly the same address the main NPA book itself shows, single-sourced, with nothing separate to fall out of sync.

**`index.html`**: Address List's card removed from the Update Data hub grid (Reference Data: 7 files -> 6) and its full-screen panel removed; Customer Master's own panel gained an explanatory paragraph stating plainly that it's now the single, app-wide address source.

**`tools/branch-split.html`**: every user-facing status string that said "shared Address List" (the address-source label shown above the Run step, the addressStep's own sub-text, the "Skipped" summary) now says "Customer Master" instead, so Branch Split's own UI doesn't describe a feature that no longer exists. No code-path changes needed here at all -- `getSharedAddressList()`/`getSharedAddressListByCustomer()` already just read whatever the parent's bridge functions return, which is exactly why repointing the bridge in `js/app.js` was sufficient on its own.

Verified end-to-end against the real production PIN login (not simulated): `window.UPGB_getAddressList()` returns 13,302 real Account No. -> Address entries and `UPGB_getAddressListByCustomer()` returns 12,972, both built live off the real, currently-published NPA book's own `C.ADDR` field -- matching the same address-coverage figure independently confirmed earlier this session by counting `DATA.npa.rows` directly. Update Data hub shows exactly 8 cards (no Address List) with the Reference Data category correctly reading "6 files". `node --check js/app.js`, a CSS brace-balance check, and `node --check` on `tools/branch-split.html`'s extracted script are all clean. Zero console errors across the whole pass.

### Redesign: Settings → Update Data, from a flat stack of accordions to a Utility-Hub-style card grid (2026-09-25, same day)

Alok: "is update data page ko kuch utility hub ki tarah redesign karna" (redesign this Update Data page a bit like Utility Hub). Confirmed two follow-up questions directly (AskUserQuestion): each card should open its upload panel **full-screen** (not expand in place), and the cards should be **grouped into categories** the way Utility Hub's own tools already are.

The old page was a single scrolling stack: the primary daily NPA upload at top, then 9 `<details>` accordions one after another (Customer Master, Branch-wise Advance & NPA Mar/Jun, Interest Reversal, Address List, Branch Contacts, Daily PNPA, KCC Overdue, Special Note, Lok Adalat) -- functional, but visually flat and increasingly long as more upload types were added over the session.

**`index.html`**: the 9 `<details class="master-upload-details">` blocks became 9 plain `<div class="update-data-panel" id="...same id as before...">` blocks, `hidden` by default -- every element *inside* each one (file inputs, drop zones, template-download buttons, status `<span>`s, description text) kept its exact original `id`, so none of the existing upload/parsing code needed to change at all. A new `#updateDataHub` sits above them: a `.utility-grid` of two `.utility-category` sections -- "Daily Files" (Daily PNPA, KCC Overdue) and "Reference Data" (the other 7) -- reusing the exact same `.utility-card`/`.utility-category-heading`/hue-token classes (`tool-teal`, `tool-jade`, `tool-blue`, `tool-violet`, `tool-rose`, `tool-terracotta`, `tool-cyan`, `tool-indigo`, `tool-gold` -- all 9 existing hues, one per card) Utility Hub's own full-width page already defines, rather than inventing a parallel design system. Each card's subtitle is a live status mirror (e.g. "not loaded yet" / "✔ 1,234 record(s) loaded"), not just a static description, so Alok can see at a glance which files are already loaded without opening each one. Version History stays exactly as it was, a `<details>` accordion at the bottom -- it's a log, not an upload panel, so it wasn't part of this redesign's scope.

**`js/app.js`**: two new functions, `openUpdateDataPanel(panelId)` (hides the hub grid, shows one panel) and `closeUpdateDataPanel()` (the reverse, wired to each panel's new "← Back to Update Data" link) -- `toggleUpdateModal(true)` now also calls `closeUpdateDataPanel()` so the modal always reopens on the hub grid rather than wherever it was left. Each tile's live status mirror is kept in sync with its panel's real status label via a small `MutationObserver` per pair (9 total, wired once at init) rather than editing the ~10 existing call sites that already set those labels' text from various upload handlers scattered through the file -- whichever one fires, the tile updates automatically, with zero risk of drifting out of sync or missing a call site.

**`css/styles.css`**: a small new block (`.update-data-panel`, `.update-data-back-btn`) for the full-screen panel's own heading/back-link chrome -- the card grid itself needed no new CSS at all, since `.utility-grid`/`.utility-category`/`.utility-card` are reused verbatim from Utility Hub's own stylesheet rules.

Verified end-to-end with Playwright against the real production site's own PIN login (this modal sits behind Admin gating for its *open* button, but not behind any additional auth once past the splash screen, so this is a genuine, not simulated, test): 9 cards render across the right 2 categories with the right titles; clicking a card hides the grid and shows exactly that one panel, with its real file input/template-download button/status div all present and unchanged; the panel's "← Back" link returns to the grid; manually updating a panel's status label live-updates its tile's mirrored text; reopening the modal after navigating into a panel resets back to the grid. Screenshotted at desktop (1280px) and phone (390px) width, both themes -- the fixed-width `.utility-card` (296px) wraps to 2 per row inside the 720px modal instead of 3-4 on Utility Hub's own full-width page, and stacks to 1 per row on a phone, exactly as intended, with zero console errors across every combination. `node --check js/app.js` and a CSS brace-balance check both clean.

Alok asked for a big new feature on the branch-facing Recovery Dashboard portal (dashboard.alokmittal.net): when a branch manager logs in, show which accounts recently slipped into NPA -- Today / This Week / This Month -- split by KCC vs Non-KCC, plus the branch's current NPA total and its gap against the existing March baseline.

Investigated his real "Daily PNPA" .xlsb export directly (41,026 whole-bank rows, 636 Hathras) before building anything, rather than guessing column names. Found that the file's "Cust NPA Date" column is a **record of when an account already became NPA**, not a forward prediction -- confirmed directly: every row with a non-zero Balance Amount has this date populated, and every zero-balance row (flagged for unrelated reasons like Limit Review, No Credit for 90 Days, or Term Complete) doesn't. Confirmed with Alok directly that this is exactly what he wants: a "recently slipped" tracker keyed off this date, not a genuine future-slippage projection (which this file has no data for).

**`js/app.js`**: `PC` (the PNPA column map) gets one new field, `CUSTNPADATE` (index 10) -- purely additive, so the existing hidden `pnpa` nav view and the standalone "Daily PNPA Summary" Utility Hub tool, which only ever read the first 10 indices, are completely unaffected. `parsePnpaRows()` now also reads a `Cust NPA Date` column (optional -- a file without it still parses exactly as before) and stores it the same way KCC Overdue's own Cust NPA Date is stored: `fmtDate()`-formatted at parse time, re-parsed via `toDate()` wherever the recency calculation needs it back as a real Date.

Settings → Update Data → Daily PNPA panel copy updated to mention the new Recovery Dashboard use of this field.

**Recovery Dashboard** (`dashboard.alokmittal.net`, separate repo `mittalok-creator/RECOVERY-DASHBOARD`) -- new "PNPA Slippage" nav tab:
- Fetches `data/pnpa.json` cross-origin from `npadashboard.alokmittal.net` (same already-public, CORS-open pattern the KCC Overdue tab already uses there) -- no separate upload on this portal, same "read-only, single source of truth" design as the rest of it.
- Three tabs (Today / This Week / This Month), rolling day-count bands off each account's Cust NPA Date relative to the viewer's own clock (0 days / 1-7 days / 8-31 days ago) -- not calendar-aligned, simpler and unambiguous.
- Each tab shows a KCC vs Non-KCC summary (account count + ₹ total, scheme code CC004 = KCC, matching the app's existing convention) and a full account list (Account No, Name, Balance, CADU, Cust NPA Date), branch-locked to the logged-in Sol ID exactly like the Dashboard/KCC Overdue tabs already are (same `KCCOV_BRANCH_SOL`-based branch-name resolution technique).
- A top strip shows "Aaj ka NPA" (the branch's current total NPA O/S, reusing `computeDashboardStats()`) and "Last March se Gap" (reusing the existing `DATA.branchAdvances`/`npaMar26` baseline and the exact same over/under-baseline comparison Dashboard's own corner-stats already show). "Is Month ka Target" is shown as a clearly-labeled placeholder ("Set nahi hai") -- deliberately deferred, since no per-branch forward target exists anywhere in this app's data yet; needs a follow-up round once Alok defines how that number gets in (most likely a new optional column on the existing Branch Advance upload, following its own `npaMar26`/`npaJun26` naming convention).
- Each account row has an editable Remark field -- **device-local only for now** (`localStorage`, keyed by Account No, same non-published pattern as this app's own OTS Amount/Interest Reversal overrides), per Alok's own explicit instruction ("abhi provision karo, mere paas Synology ka home server hai, future mein use configure karenge") -- clearly labeled in the UI as saved on this device only, not yet synced across branches/devices.
- Clicking an account opens the existing read-only Quick Account Detail card (`showQuickAcctDetailByAcct('pnpa', acctNo)`) -- no new detail UI needed.

**Explicitly deferred, per Alok's own framing ("baaki add-ons baad mein karenge")**: "Is month ka target" and "next March se gap" (no data source yet), redesigning the main app's own data-upload UX to be drag-and-drop like the Utility Hub tools, and a real synced backend for the Remark field (Alok's own Synology NAS, once he sets it up).

## Verification

- Real-file verification, not synthetic: ran the actual `xlsx.full.min.js` (same library the browser loads) against Alok's real `.xlsb` file in a Node harness -- confirmed `parsePnpaRows()` parses exactly 35 Hathras rows (matching an independent Python/`pyxlsb` analysis of the same file byte-for-byte), all 35 correctly carrying a Cust NPA Date.
- Bucketing logic verified against those same 35 real rows: with "today" set to the real current date, all 35 (dated 03-04 Sept) correctly land in the "This Month" band (21 days ago); branch-scoped aggregation for one real branch (Hathras Agra Road, 6 accounts) reconciles exactly against a hand count.
- Full Playwright pass against the real Recovery Dashboard code (network-intercepted to serve the real parsed 35-row fixture in place of production's `data/pnpa.json`, so this exercises the actual shipped rendering/aggregation code, not a mock): login as a real Sol ID, correct branch-locked tab counts (0/0/6), correct KCC/Non-KCC split and ₹ totals, Remark saves and survives a tab switch, clicking an account opens the correct Quick Account Detail card. Zero console errors throughout.
- `node --check` clean on both repos' `js/app.js`; CSS brace-balance clean on both.
- No-regression check: `PC` indices 0-9 unchanged, so `pnpaBucketOfRow`, the hidden `renderPnpaDashboard`/`renderPnpaDashboardBody`, and the standalone Daily PNPA Summary tool (which doesn't even read this far into the row) are all untouched and still correct.
...
Direct data analysis of the real reference file confirmed Scheme Code and Reasons are in exact 1:1 correspondence (CC004↔KCC-Disbrsmnt-36, CC043↔KCC-Disbrsmnt-15, OD023↔KCC-Disbrsmnt-24, zero exceptions across 6,802 rows) -- meaning today's 3 scheme buckets and the standalone tool's 3 reason buckets are the same 3-way partition of the same rows, so a genuine merge was sound, not just a relabel. This meant the new views could bucket entirely via the already-proven `kccOverdueBucketOf()` (keyed on Scheme Code) and never need to look at the Reasons column at all -- structurally ruling out a "15/24/36 backwards" mixup.

**Parser** (`parseKccOverdueRows`): no change to the `KC` row shape (the published, encrypted `data/kcc-overdue.json` stays byte-for-byte compatible) -- the raw daily "Data" export was already a strict superset of every column it reads. Only the sheet auto-pick regex was broadened (now also matches a tab literally named "Data") and the "missing column" error text now references "the daily KCC 'Data' export layout." The Sol column needed by the new All Branches view reuses the existing `KCCOV_BRANCH_SOL` lookup off the branch name -- no new field needed.

**Three new views** added to the "KCC Overdue" tab's toggle row, alongside the existing Branch Summary/Datewise Calendar: **F.Y./Month Summary**, **Branch Report** (reuses the existing toolbar Branch filter, no new dropdown), and **All Branches Overview**. All three reuse the same toolbar filters (Branch/F.Y./date) as the existing views, with the scheme hero-KPI row hidden while any of them is active. Aggregation/rendering ported from the standalone Utility Hub tool (`kccovAggregateFyMonth`/`kccovAggregateBranches`/`kccovBuildFySectionTable` etc.), with one real `<table><thead>/<tbody>` per F.Y. section so both native print pagination and PDF export repeat each section's own header correctly. A blank F.Y. now lands under "Unspecified" (sorted last) instead of being silently dropped. Drill-down reuses the existing `showKccovListModal` infrastructure.

**New Excel export** (`exportKccOverdueBifurcation`, additional to the existing WYSIWYG Branch Summary export): 4-sheet workbook (Report/Data/Branch Report with an in-Excel branch dropdown/All Branches), ported from the standalone tool's already-shipped, already-verified version.

**Print/PDF**: Print reuses the app's existing `#printArea`/`printWithPageSize()` mechanism (A4 landscape). Save-as-PDF ports the standalone tool's row-aware, header-repeating capture technique (`kccovCaptureTablePages`) using this app's own lazy `ensureHtml2Canvas()`/`ensureJsPDF()` loaders.

**Not touched**: `js/publish.js`/the encryption pipeline (row shape unchanged, so nothing here needed editing), and `tools/kcc-overdue-summary.html` (stays live exactly as-is, per Alok's explicit choice, as a no-login/no-publish fallback).

**Verified**: a Node harness (functions copied verbatim from `js/app.js`, run against a synthetic fixture matching the real "Data" file's column layout) asserted 32/32 checks -- parser accepts the raw layout, Region/scheme-code filtering, the scheme→bucket mapping direction (CC004→kcc, CC043→kccah, OD023→od023, explicitly guarding the "backwards" risk), blank F.Y. → "Unspecified", and aggregation totals reconciling between the F.Y./Month view and the Branch view. `node --check js/app.js` clean throughout. A Playwright boot check confirmed zero console errors and the pre-login splash still renders. The real reference file used for earlier rounds' verification was not available in this session to re-run the exact-totals cross-check (6802/6063/629/110) -- noted honestly rather than claimed. No live end-to-end test of the authenticated `kccov` view itself is possible in this sandbox (PIN/login-gated).

### Feature: Excel "Branch Report" sheet gets a real dropdown -- every branch in one export, not a static snapshot (2026-09-24, later same day)

Alok, after confirming everything else from the last two rounds was working: "sab bhadiya hai bas 1 kami bachi hai jab excel export kar rahe hain tab branch report main keval 1 branch ka data aa raha hai jabki mujhe yahan sari branches ka data chahiye with dropdown of branch name" -- the exported "Branch Report" sheet only ever showed the one branch selected on-screen at export time; he wants every branch in the same export, selectable via a dropdown.

This was a known simplification from the tool's original build: the reference VBA macro's own "Branch Report" sheet is formula-driven (a dropdown cell + live `COUNTIFS`/`SUMIFS` reading off a raw "Data" sheet), not a static dump -- the shipped version took the simpler "snapshot whichever branch is currently selected in the browser" path instead. Alok's feedback confirms he wants the real macro-equivalent behavior.

New `Data` sheet added to the export: every parsed row (Sol, Branch, Account No, Balance Amount, Reason -- the full label text, Cust NPA Date, F.Y., Month Label), plus, in a spare column, the sorted list of distinct branch names -- this is what the dropdown's data-validation range points at rather than an inline list, since Excel's inline list-validation formula has a hard ~255-character limit that 55 real branch names blow straight past; a range reference has no such limit. `writeBranchReportSheet()` replaces the old one-branch sheet: same F.Y./Month band+header layout as the `Report` sheet (the existing `writeFyBand`/`writeTwoRowHeader` helpers, reused verbatim), but every data cell is now a `COUNTIFS`/`SUMIFS` formula filtered by the dropdown cell (`$B$2`) plus that row's fixed F.Y./Month/reason criteria, and each F.Y. TOTAL row is a plain `SUM()` over its own section. Changing the dropdown in Excel recalculates instantly -- no macro, no re-export, plain formulas. The dropdown pre-fills with whichever branch was selected on-screen at export time, for convenience, but every branch is reachable from it. `wb.calcProperties = {fullCalcOnLoad:true}` set so a viewer whose calculation mode isn't Automatic still sees correct figures immediately on open rather than stale/blank cells. `Report` and `All Branches` sheets are unchanged -- they already cover every branch, just not filterable to one.

Verified the exported file's raw structure directly (4 sheets: `Report`, `Data`, `Branch Report`, `All Branches`; `Data` holds all 6,802 real rows from the reference file plus the 55-branch dropdown list; the dropdown's validation formula is exactly `Data!$J$2:$J$56`; the dropdown pre-fills with the on-screen-selected branch, tested with BAMNAI) and read every generated formula string directly, confirming correct sheet/column references and criteria text matching exactly what's stored in the `Data` sheet's own columns. Attempted to force a live recalculation via `soffice --headless` (LibreOffice) to fully close the loop, but it fails to load even a trivial one-cell test file in this sandbox -- confirmed an environment limitation, not specific to this export, rather than assumed. Worked around it with an equally rigorous alternative: read the `Data` sheet's rows back out and independently reimplemented `COUNTIFS`/`SUMIFS`' exact filter semantics in a plain script, then cross-checked the result (BAMNAI, F.Y. MAR-27: 54 rows, ₹186.48 Lakh) against the same figure computed completely independently, straight from the original uploaded reference file -- both match exactly, confirming both halves (the `Data` sheet holds the right rows, and the formula logic reads them correctly). `node --check` on the extracted inline script clean.

### Fix: KCC Overdue Summary's Print/PDF was broken across pages -- real thead/tbody per section, row-aware pagination, richer color (2026-09-24, later same day)

Alok, after seeing the just-shipped Save-as-PDF output for real: "ismain har page par header fix karo and jara colorful banao, and app main is tab ka audit karo tumhe bahut galtiyan banegi" (fix the header on every page in this [PDF], make it a bit more colorful, and audit this tab in the app -- you'll find a lot of mistakes), attaching the actual `KCC_Overdue_All_Branches_20260924.pdf` he'd generated.

**Read the attached PDF directly rather than assuming what was wrong.** Page 2 had no header at all, and one row (SADABAD) was sliced exactly in half across the page boundary, its numbers visually garbled -- confirming both halves of "header repeat" and "no row-splitting" (the print/PDF round's own honestly-flagged "known limitation" from earlier the same day) were real, user-facing problems, not theoretical ones. That earlier version rasterized the WHOLE table into one canvas and blind-sliced it by raw pixel height with no awareness of row boundaries at all.

**Took the audit ask seriously rather than perfunctorily** -- tested paths never actually exercised before (CSV upload, the multi-sheet picker, synthetic edge-case data: balances sitting exactly on the 5L/10L bracket boundary, an unrecognized Reasons value, a blank Cust NPA Date, a blank F.Y., a blank branch name, a zero and a negative balance) via Playwright against the real tool, not just re-reading the code. Found and fixed two real, if minor, issues this surfaced: a blank branch name rendered as a blank, confusing-looking row in All Branches and an indistinguishable blank entry in the Branch Report dropdown -- both now show "(No Branch)"/"—" instead. Also rendered an actual native-print PDF (not just checked computed `visibility`, which is what let the header-repeat issue ship in the first place) for the All Branches tab against the REAL reference file, and found a second, more serious bug than the one Alok had already reported: the Sol/Branch column's own `position:sticky` (left:0, for the on-screen frozen first column) was never neutralized for print at all -- Chromium's print engine tore those cells out of their row entirely and dumped them as a separate block after every numeric column, on every row, garbling the whole table. The original print-CSS fix from earlier the same day only neutralized a hand-picked list of sticky selectors (`th`, `.month-cell`, `.month-head`, the F.Y. band) and simply missed this one -- exactly the kind of mistake a selector allowlist invites.

**Fixed properly rather than patching the selector list again.** Restructured both `renderBifurcationTable()`/`buildFySectionTable()` (F.Y./Month Summary, now one real `<table><thead>/<tbody>` **per F.Y. section** rather than one giant flat table) and `renderAllBranchesTable()` (one `<table><thead>/<tbody>`) to use real semantic markup instead of a flat pile of `<tr>`s. This lets the browser's own native print pagination repeat each table's `<thead>` on every page it spans and never break a page mid-row, for free -- no more bespoke slicing logic to get right. The sticky-kill for print is now a blanket rule (`.bifurcation-table, .bifurcation-table *{position:static!important}`) instead of a selector list, and -- learning from the miss above -- applied unconditionally (not only inside `@media print`) so it also covers the Save-as-PDF capture path, not just an actual browser print.

**Save-as-PDF rewritten to match**: no more one-shot rasterize-then-slice. `captureTablePages()` now measures each table's own real thead height and a real row height (`getBoundingClientRect()`), computes how many `tbody` rows fit on one landscape-A4 page with a one-row safety margin, and captures the table **page by page** -- hiding out-of-range rows via `style.display`, capturing with `html2canvas`, then un-hiding for the next chunk -- so the header is genuinely re-rendered fresh on every page (not sliced/composited) and a page break can only ever fall between two rows, never through one. Each F.Y. section always starts its own fresh PDF page (mirrors the native print CSS's `break-after:page` on `.fy-section-table`), matching the reference macro's own one-section-per-page report layout.

**Colorful, per Alok's ask**: new `[data-printing]`-scoped CSS (deliberately *not* wrapped in `@media print`, so the same rules apply to both an actual browser print and the `html2canvas` capture) gives the header rows a solid deep teal (`#0E6B57`) background with white text, a visible light-mint zebra tint on alternating data rows, and a stronger warm highlight + border on F.Y. TOTAL/Grand Total rows -- replacing the on-screen table's much subtler grayscale header/zebra, which read as flat once actually printed. The on-screen, day-to-day view is untouched -- this is scoped entirely to Print/PDF.

Verified against the real reference file (`TODAYS_MACRO.xlsx`, 6,802 rows), not synthetic data alone: both native print (via a rendered print-media PDF, this time actually opened and read page by page, not just computed-style-checked) and Save-as-PDF now show a full, correctly-colored header on every page for both the F.Y./Month Summary (one page per F.Y. section: MAR-27's 7 months on page 1, MAR-28's 12 months on page 2, each with its own band+header+TOTAL row) and All Branches Overview (55 branches across 2 pages, split cleanly between SALEMPUR/SASNI with no row torn in half, Grand Total intact on the final page) -- and the Sol/Branch columns stay correctly attached to their own row's numbers throughout. Re-ran the branch-dropdown-then-export-to-Excel regression once more (still correct, unaffected by this round). On-screen sticky-header behavior (the previous round's own fix) re-verified unaffected by the thead/tbody restructuring -- `position:sticky` computes off the nearest scrolling ancestor (`.bifurcation-scroll`), not the `<table>` boundary, so splitting one table into several sibling tables changes nothing about it. `node --check` on the extracted inline script and a brace-balance check on the tool's own `<style>` block both clean.

### Fix: KCC Overdue Summary clipped to a fixed-height box (3 of 4 bugs), sticky 2-row header stacking, plus new A4 Print/PDF (2026-09-24, next day)

Alok, right after using the just-shipped tool: "app to ban to gaya but exported excel main branch report main branch ko drop down se select karke us branch ka data aana hai wo nahi aa raha keval 1 hi branch ka data aa raha hai, and app main scroll nahi ho raha left to right and niche total row show nahi ho rahe, header ki 2 row main se keval 1 hi row freeze hai dusri hide ho ja rahi hai fix these issues, and app se hi monthly summary and all branch overview ko a4 sheet main set kar k print and pdf direct ka option do" -- four bugs plus a new print/PDF ask, all in one message.

**Root cause for three of the four bugs at once, found by direct code reading, not assumed**: `css/styles.css`'s default `.tool-frame-wrap{height:calc(100vh - 210px);...overflow:hidden}` rule clips any Utility Hub tool that doesn't have its own matching `#view<Name> .tool-frame-wrap{height:auto;...}` override -- every self-resizing tool (one that grows its own iframe via `resizeFrame()`, reading `document.documentElement.scrollHeight`) needs this override or it's stuck in a fixed ~480-700px box with no way to scroll to, or even see, anything below that fold. `tools/kcc-overdue-summary.html` already had its own `resizeFrame()` (copied from the same pattern every sibling tool uses), but the matching CSS override for `#viewKccOverdueSummary` was simply never added when the tool was built two days ago -- confirmed by an already-existing code comment on this exact same rule for NPA SOL Summary (a prior round), which documents this identical failure mode happening once before: "Missing this on first ship was the actual 'scroll nahi ho raha hai' bug... clipped the table at a fixed height with no scrollbar at all... just this override never having been added for the new view." That one missing block plausibly explains "TOTAL rows not showing" (literally clipped off-screen), "can't scroll left-right" (can't even reach the table's own horizontal scrollbar past the clipped fold), and very likely "branch export always shows 1 branch" too -- the Branch Report tab's own dropdown sits below that same fold, so a selection may never have reliably registered. A separate empirical Playwright test (done before touching any code) already proved `exportToExcel()`'s own JS logic is 100% correct -- selecting a branch then exporting always produces a "Branch Report -- <that branch>" sheet -- so this wasn't a second, separate export bug, just a symptom of the same clipping.

Fixed by adding the missing `#viewKccOverdueSummary .tool-frame-wrap`/`.tool-frame` override to `css/styles.css`, immediately after the existing `#viewBranchSplit` block -- identical pattern to every sibling tool.

**A second, separate, smaller bug**: both header rows' `<th>` cells (group labels, then A/C Count/Amount sub-labels) carried the exact same `position:sticky;top:0` -- along with the F.Y. band row above them, three tiers all competing for the same sticky position, so header row 2 rendered hidden directly behind row 1 instead of stacking below it, matching Alok's exact description ("2 row main se keval 1 hi freeze hai, dusri hide ho rahi hai"). Fixed by giving each header `<th>` a `hdr-row1`/`hdr-row2` class (added in `renderBifurcationTable()`/`renderAllBranchesTable()`) and distinct `top` offsets measured against this table's own real row heights (band ~32px, each header row ~29px) -- the F.Y./Month and Branch Report tables (which have a band row above their header) use `32px`/`61px`; the All Branches table (no band) uses `0`/`29px`. The bounded `.bifurcation-scroll{overflow:auto;max-height:560px}` box itself is unchanged -- it's the only scrolling context inside the iframe that makes "frozen header while scrolling" possible at all; removing it would have eliminated the freeze feature Alok explicitly asked to keep working, not fixed it.

**New feature**: "Print" and "Save as PDF" buttons added to the F.Y./Month Summary and All Branches Overview tabs only (not Branch Report -- not asked for). Mirrors the exact, already-proven pattern from the main app's own OTS one-pager (`printWithPageSize()`/`canvasToPdfBlob()`/`shareOtsPdf()` in `js/app.js`), reimplemented as a standalone copy in this tool's own script (self-contained, no shared imports, same convention every tool file already follows) -- **landscape A4**, not portrait, since these tables run 13-14 columns wide. Print sets a `data-printing="fymonth"|"allbranches"` attribute on `<body>` (cleared on `afterprint`) that a new `@media print` block keys off, mirroring the main app's `#printArea`-only-visible whitelist pattern; sticky positioning is neutralized to `static` during print (only meaningful inside the bounded on-screen scroll box, which print un-clips). Save-as-PDF captures the `<table>` element directly with html2canvas (not the clipped `.bifurcation-scroll` wrapper -- html2canvas renders the full target regardless of an ancestor's own overflow/max-height, so no temporary style mutation/restore is needed here, unlike `shareOtsPdf()`'s own normally-hidden `#printArea`), slices into page-height strips via the same jsPDF logic the OTS PDF share already uses if the table is taller than one page, and downloads via this tool's existing `anchorDownload()`. Vendored `html2canvas.min.js`/`jspdf.umd.min.js` directly (both already shipped for the main app's own lazy-loaded PDF share) rather than lazy-loading, matching this tool's existing eager-vendor-script convention. Known, accepted limitation: header rows aren't wrapped in a real `<thead>` (they repeat per-F.Y.-band by construction), so a single F.Y. section that itself spans more than one printed page won't repeat its header mid-section -- can revisit if Alok hits it in practice.

Verified: `node --check` on the tool's extracted inline script clean; brace-balance clean on both `css/styles.css` and the tool's own `<style>` block; Playwright against a same-origin fake-parent embed (critical -- a `file://` parent breaks `window.frameElement` via cross-origin restrictions, so the earlier standalone-only test couldn't have caught this class of bug) confirmed `.tool-frame-wrap`'s computed `overflow` is no longer `hidden` and the iframe's own inline `style.height` now matches its full content height, with the F.Y. TOTAL rows, Grand Total row, and the Branch Report dropdown all now visible and reachable; re-ran the branch-select-then-export regression check end-to-end post-fix; screenshotted the sticky-header fix mid-scroll confirming both rows stay visible, correctly stacked, band on top, in both the F.Y./Month (with band) and All Branches (without band) tables; triggered both Print buttons under Playwright's print-media emulation confirming the right elements compute visible/hidden per `data-printing` value with no console errors; triggered both Save-as-PDF buttons and confirmed non-trivial, cleanly-openable PDF files were produced for both tables.

### Feature: new Utility Hub tool — "KCC Overdue Summary" (2026-09-24)

Alok, uploading a VBA macro (`KCC_Report_Macro_4.bas`) and a sample data workbook (`TODAYS_MACRO.xlsx`) as reference: "utility main ek aur tab ad karo kcc overdue summary. ye data chahiye mujhe with and reference data is also provided" -- add one more tab to the Utility Hub, "KCC Overdue Summary," built off the macro's own report logic.

**Naming check first**: the app already has a completely unrelated nav tab also called "KCC Overdue" (Settings → Update Data upload of a pre-filtered Head Office export, its own dashboard/calendar view, `data/kcc-overdue.json`, needs Publish to go live). Investigated (Explore agent) to confirm the two are genuinely different features -- different data shape (this new tool reads the *raw* daily "Data" export, not a pre-filtered one), different purpose, no shared code or storage. Confirmed with Alok directly (AskUserQuestion): keep the name "KCC Overdue Summary" anyway for the new Utility Hub tool, since it lives in a different part of the app. Also confirmed: build both of the macro's reports (F.Y./Month bifurcation + single-branch drill-down) plus a third table he asked for beyond the macro (an all-55-branches-at-once overview), and both an on-screen dashboard and an Excel export styled like the macro's own.

Read the macro in full (325 lines) and inspected the real reference workbook directly rather than assuming: its "Data" sheet's real header row matches the macro's hardcoded column-letter assumptions exactly (Balance Amount=I, Reasons=Q, Cust NPA Date=T, F.Y.=U), confirming the macro's own logic is a faithful spec -- Reasons has exactly 3 real values (`KCC-Disbrsmnt-15/24/36`, no "other" bucket needed in practice), F.Y. values carry literal embedded quote characters (`"MAR-27"`) the app already has a `stripQuoteChars()` precedent for. Despite the letter-match in this sample, the new tool parses by header text (not fixed column letters), matching this app's own established convention (`parseKccOverdueRows`/`buildCustomerMasterMap` in `js/app.js`) for resilience against a differently-ordered export.

New self-contained `tools/kcc-overdue-summary.html`, built on the same Utility Hub tool skeleton every sibling tool already uses (theme-synced via `localStorage['upgb-theme']`, vendored `xlsx.full.min.js` + `exceljs.min.js`, own inline CSS token set) -- single-screen layout (upload → optional sheet picker → summary strip → a 3-way tab bar), not a multi-step wizard, since there's only one real input. Parses the "Data" export into row objects (reason bucketed into `d15`/`d24`/`d36`, month/F.Y. derived from Cust NPA Date via the same `toDate()`/no-`cellDates:true` convention the main app already settled on), aggregates into 6 column-groups per the macro (3 reason buckets, "All Data", "5 Lakh+ A/C", "10 Lakh+ A/C") across three views sharing one rendering function: F.Y./Month Summary, a branch-dropdown-driven Branch Report (same function, pre-filtered rows), and an All Branches Overview (one row per branch, no month dimension). Every bucket cell is clickable, opening a drill-down modal listing the underlying accounts. "Export to Excel" (ExcelJS, needed for the merged/colored cells a plain XLSX write can't do) produces a 3-sheet workbook (`Report`, `Branch Report`, `All Branches`) visually matching the macro's own colored F.Y. bands, two-row group headers, and highlighted F.Y. TOTAL rows.

Wired into the Utility Hub the same static way every tool is: new `.utility-card` tile (new `tool-jade` hue -- the 8 existing hues were all already taken) in the "Summaries & Reports" category, a new `.view`/iframe section in `index.html`, `UTILITY_CHILD_VIEWS`/`TOOL_IFRAME_BY_VIEW` entries in `js/app.js` (both fully generic, no other JS changes needed), tool-count bumped 8→9.

Verified the parsing/aggregation logic directly against the real reference workbook (not a synthetic stand-in) by extracting the tool's actual functions and running them in Node against `TODAYS_MACRO.xlsx`: 6,802 rows kept, 0 skipped, reason split 6,063/629/110 and F.Y. split 4,180/2,622 -- both exactly matching independent inspection of the source file -- and the F.Y.-total/grand-total reconciliation checks out (sum of all months' `all.cnt` across both F.Y.'s equals `6802`). `node --check` on both extracted inline scripts clean; CSS brace count balanced.

### Follow-up: RM Scale badge moved inline (between Name and share icons), bolder/bigger, real gap between the two dates (2026-09-24, same day)

Alok's immediate follow-up after the badge shipped: "NAME AND SHARE TAGS K BICH MAIN ALIGN KARO NEXT ROW ADD NAHI KARO TEXT KO THODA BOLD AND BADA KARO DONO DATES K BICH MAIN BHI KUCH GAP DIKHO" -- align it between Name and the share icons (same row, not a separate one below), bolder/bigger text, a visible gap between the two tenure ranges. Confirmed directly (AskUserQuestion) how this should behave on a narrow phone, since the header row was already full there: desktop -- inline in the same row; mobile -- wraps down to its own line only when there isn't room.

Moved `.rm-scale-badge` from a sibling `<div>` below `.detail-headrow` to inside it, between `.detail-headtext` and the first `.share-btn` -- matching DOM order to "between Name and icons" directly. Split the two tenure ranges (previously one string joined by "&middot;") into two separate `<span class="rm-scale-tenure">` spans inside a new `.rm-scale-tenure-group` flex wrapper, so a real CSS `gap` sits between them. Bumped `.rm-scale-title` (11px/800 -> 13px/800) and `.rm-scale-tenure` (11px/600 -> 12.5px/700).

Mobile wrap mechanism: `.detail-headrow` gains `flex-wrap:wrap` (was `nowrap`), and inside the existing `@media (max-width:859px)` block, `.rm-scale-badge` gets `order:5;flex-basis:100%`. This is the one combination that reliably achieves "inline when there's room, but cleanly drops to its own full-width line when there isn't" *without* dragging the icon buttons down with it -- a `flex-basis:100%` item can never share a line with anything else (it always claims the full row width), so ordering it *after* the three `.share-btn` icons guarantees back-button + name + icons stay together on the row above it, and only the badge itself wraps. (Considered and rejected: `flex-basis:100%` applied globally, or left at natural DOM order without a mobile-only `order` override -- both either kill the desktop inline placement or strand the icon buttons on their own separate third line on mobile.)

Verified visually, not assumed -- reused the same local Playwright harness (real header markup + real `css/styles.css`), screenshotted at common desktop widths (1280px, 1440px: full name visible, badge inline with a clear gap between the two dates) and confirmed the graceful-degradation edge case at an unusually narrow ~900-1000px "desktop" width (name truncates further via its own pre-existing ellipsis mechanism, nothing new breaks), plus phone width (~390px, dark and light): back-button+name+icons stay together on the top row exactly as before, badge cleanly wraps to its own line below with the two dates stacking naturally when tight. `node --check js/app.js` and a CSS brace-balance check both clean.

### Feature: fixed "RM SCALE - V" tenure badge on the Loan Detail header (2026-09-24)

Alok, with a screenshot of the Loan Detail screen's header: "OTS CALCULATOR KI IS PATTI PAR 'RM SCALE - V' TENURE 29-03-2021 TO 31-03-2022 AND 23-09-2024 TO 11-09-2026 LIKHA HUA AAYE KUCH IS TARAH SE KI ALAG SE EMBOSED HO KAR DIKHE" (on this strip, "RM SCALE - V" with two tenure date ranges should appear, in a way that shows separately/embossed). Confirmed directly (AskUserQuestion): fixed text, identical on every account's Loan Detail screen, not tied to any per-account data (no such concept -- RM designation/tenure -- exists anywhere in the codebase); on-screen only, not the OTS one-pager print/WhatsApp PDF.

Added a new `.rm-scale-badge` line to `openDetail()`'s header template (`js/app.js`), sitting inside `.detail-head` right below the existing `.detail-headrow` (back button/name/Cust ID/export-print-WhatsApp icons) -- that row was already visually full (`.detail-headtext` is the only `flex:1` element, the three icon buttons are fixed-width), so the badge needed its own line rather than squeezing into the row. Plain hardcoded text, no new DATA field or Settings UI -- matches this app's existing precedent for fixed institutional text (e.g. "Uttar Pradesh Gramin Bank" / "Regional Office Hathras" hardcoded directly in the OTS one-pager's own header).

Styled as a translucent chip sitting on `.detail-head`'s own themed gradient background, reusing the exact same "chip on head-grad, correct in both themes" recipe already solved for the `.back-btn`/`.share-btn` icons on this same header (`rgba(255,255,255,.06)` dark / `rgba(33,33,46,.05)` light, matching border tints), plus an `inset 0 1px 0` top-rim highlight and a drop shadow for the "embossed, showing separately" look -- an app-wide `--emboss-rim` token exists already but is built specifically for `.utility-card`'s card-shaped background (`--card`), not this header's own `--head-grad` background, so a self-contained variant was used instead rather than reusing that token directly against a background it wasn't designed for. Wraps onto its own line under the "RM SCALE - V" label at phone width via `flex-wrap`, with the same mobile-breakpoint size trim already applied to the rest of this header.

Verified visually rather than assumed -- this screen needs a live logged-in session, so the exact real header markup + the real `css/styles.css` (not a copy) were served standalone via a local Playwright harness and screenshotted at desktop and phone width, in both dark and light theme (4 combinations): badge renders as a clearly distinct, legible chip in all four, no overflow/clipping, wraps correctly on narrow screens. Confirmed absent from `renderPrintView()`'s own output (on-screen only, per Alok's explicit answer) -- and even if it weren't, `@media print`'s `#printArea, #printArea *{visibility:visible}` rule would hide it anyway, since it lives in `.detail-head`, not `#printArea`. `node --check js/app.js` and a CSS brace-balance check both clean.
...
Investigated (Explore agent, then a Plan agent that traced the vendored SheetJS 0.20.3 source directly rather than assuming). A worksheet-selection dropdown already existed, and was already positioned before Region -- but wasn't doing what it should: `handleMasterFile()` called `XLSX.read()` on the *entire* uploaded file with no sheet restriction, eagerly parsing every sheet's cell data into `state.wb` before the dropdown was even populated, then auto-loaded the first one. For a 4-sheet 186MB file, all 4 sheets got fully parsed before Alok ever got a chance to pick the one he actually needed. Separately: there is no file-size cap anywhere in this codebase (exhaustive grep, confirmed 100% client-side, zero network calls in this tool) -- "30MB" doesn't correspond to any real constant here. The real, already-documented constraint is the "10-million-cell SheetJS parsing ceiling" fix from two days ago (`dense:true`, raising the practical ceiling to ~50M cells but only verified up to a 7M-cell repro) -- a 186MB/4-sheet file plausibly still hits that ceiling, and unnecessarily so, since every sheet's cells counted against it even though only one is ever used per split.

Fixed by making sheet-name discovery genuinely free and deferring real cell-data parsing to only the one sheet requested, using SheetJS's own lazy-read options -- verified against the actual vendored source, not the docs: `bookSheets:true` returns `SheetNames` before the per-sheet parsing loop is ever entered (`if(r.bookSheets?m.SheetNames:r.bookProps)return m}` -- no cell data or shared strings touched, for any sheet, regardless of file size), and `sheets:[name]` skips parsing any non-matching sheet (`continue e` before the sheet-XML parser runs), with case-insensitive name matching safe against names pulled straight from the names-only pass. `handleMasterFile()` now does the cheap names-only pass first and keeps the raw `Uint8Array` in a new `state.rawWorkbookBuffer` (FileReader only ever runs once); the embedded "ADDRESS" sheet lookup gets its own small `sheets:['ADDRESS']` parse in its own try/catch (a genuine small improvement too -- a problem with that optional fallback sheet used to surface as a scary "could not read this file" error for the whole upload, now it's isolated); `loadSheet()` re-parses from the buffer restricted to just the requested sheet every time it's called (first load or a dropdown change), guarding the sheet-name-mismatch edge case the same way the existing "no Branch/Sol column" error already is, rather than an uncaught exception. Only `handleMasterFile`/`loadSheet` needed changes -- an exhaustive grep found exactly 4 references to `state.wb` in the whole file, and the other two (`buildSimpleSplitWorkbook`'s own read, and the sheet-count note) keep working unmodified since `state.wb.Sheets[state.sheetName]` still resolves correctly, just from a targeted reparse instead of a leftover reference into one giant eagerly-parsed workbook. CSV is untouched (always one implicit sheet already, nothing to defer).

Verified with a synthetic 4-sheet `.xlsx` (2 decoy sheets with real bulk data, the real Branch-Split-shaped sheet placed 3rd rather than first, plus an ADDRESS sheet 4th) via Playwright: all 4 sheet names appear in the dropdown almost immediately; the auto-loaded first (decoy) sheet correctly shows the existing "no Branch/Sol/Account No" warning rather than crashing; explicitly picking the 3rd sheet correctly re-parses and proceeds through the rest of the flow; the embedded ADDRESS sheet (4th) still resolves correctly in the final output, proving the targeted `sheets:['ADDRESS']` parse works; a full run produces correct output for both branches. Full existing regression suite (7-case flow test, SBA/Reset test, address-fallback/iframe-bridge test, the manual-address-override test from earlier today) re-run with no change in behavior. `node --check` on both extracted inline scripts clean.

**Honest limits, told to Alok directly:** this substantially reduces the odds of hitting the parsing ceiling (only one sheet's cells count against it now, not all four), and confirms there's no deliberate 30MB cap anywhere to blame -- but it can't be verified against his actual 186MB file in this environment, and won't help if his one needed sheet is *itself* large enough to approach the ceiling alone, or if the browser is failing earlier, just reading the raw 186MB file into memory (unaffected by this change either way -- a genuinely different, much bigger fix, e.g. a Web Worker, which doesn't exist anywhere in this codebase today and is out of scope for this round). Asked him to retry and, if it still fails, report which sheet he's picking and roughly its size.

### Fix: OTS one-pager print/PDF was missing the Settlement % badges the on-screen table already shows (2026-09-23, same day)

Alok: "ots one pager print karte main ye % nahi aata ise live karo" (this % doesn't show up when I print the OTS one-pager, make it live), with a screenshot of the on-screen "Settlement Progress" element (a rupee figure, a %, a progress bar, and "X% of dues · Y% of O/S").

Investigated rather than guessing (Explore agent + direct code read). The on-screen figure is `recalcLoan()`'s `settlePct`/`settlePctOs` (`js/app.js`), rendered per account in the loan table; a matching aggregate version drives the sidebar chips via `recalcAggregate()`. The "one pager" -- both the Print button and the WhatsApp PDF share -- is not a screenshot/capture of that on-screen table at all: `printOtsSheet()` (plain `window.print()`) and `shareOtsPdf()` (html2canvas → jsPDF) both render from one shared, separately-authored HTML sheet built by `renderPrintView()` into `#printArea` -- one fix here fixes both outputs. `renderPrintView()`'s own row set was simply never given the settlement percentage when it was added on-screen (a pre-existing comment already documents one other field being deliberately left on-screen-only, confirming this table has always been a hand-curated subset, not an auto-derived mirror) -- not a CSS `display:none`, not a script that failed to run, just a row that was never added.

Fixed the same way this table already handles two other inline percentages (`'Total P&L'`'s `(X%)`, `'OTS Amt as per Lok Adalat'`'s `(X%)`) rather than introducing a new visual -- no progress bars exist anywhere else in this plain particulars table, so a bar wouldn't fit the sheet's look. The `'OTS Amount'` row now appends `(X% of dues · Y% of O/S)` computed identically to `recalcLoan()`'s formula, guarded the same way the row already was (blank OTS Amount still just reads `'—'`, no stray `(NaN%...)`. The aggregate `'Total OTS Amount'` hero row in the `pv-agg` block gets the same treatment, using `totalOtsSum`/`totalDues`/`totalOS` already computed earlier in `renderPrintView()`, shown only once every linked account has an OTS Amount typed (the same "all or nothing" rule the block's neighboring hero rows already use). No CSS changes needed -- `@media print`'s `#printArea, #printArea *{visibility:visible}` already whitelists anything added here, for both the printed page and the html2canvas/jsPDF capture.

Verified with a Node reproduction of both new formatters (copied verbatim) against eight cases: a normal account (no NaN/Infinity, correct `% of dues`/`% of O/S` wording); an account with no OTS Amount yet (still reads a plain `'—'`, not `'— (NaN%...)'`); Total Dues = 0 and O/S Balance = 0 individually (neither divides-by-zero into `NaN`/`Infinity`; a zero O/S correctly omits the `of O/S` clause entirely rather than showing `0%`); OTS Amount exceeding Total Dues (the dues percentage correctly clamps at 100.0%, matching the on-screen formula's own clamp, while the O/S percentage is intentionally left unclamped, also matching on-screen); the aggregate row before/after every account has an OTS Amount typed; and an all-zero aggregate case. `node --check js/app.js` clean. This screen needs a live logged-in session to visually confirm end-to-end (unlike Branch Split, which is testable standalone) -- Alok should sanity-check the printed/shared sheet after this ships.

### Feature: Branch Split gets its own optional per-run Address upload, bypassing the shared list entirely (2026-09-23, same day)

Alok, after the Publish-race fix shipped and his Customer Master file was confirmed live: "kuch to galat ho raha hai ek kaam karo yahan split karte time hi address ki excal upload karao account no and address honge bas column a and b main" (something's still wrong -- do this instead: right at split time, let me upload an address Excel here, Account No and Address, just columns A and B). Rather than keep chasing the shared-list/Publish pipeline (four rounds deep already today), he wants a direct, local escape hatch inside Branch Split itself.

This is a deliberate, explicit reversal of one piece of the original Branch Split redesign, where Alok himself asked to remove the address override entirely and make it fully automatic (the "silent -- no user-facing control" comment, now updated). Confirmed with him directly (AskUserQuestion) before building: this should be its own small **optional** step in the flow (not just a quiet link), and the uploaded file should **win over** the shared Address List/embedded ADDRESS sheet wherever it has an entry.

New `addressStep` panel, inserted into `STEP_ORDER` between Branches and File name, follows the exact same `.panel-open`/`.panel-collapsed-row` step pattern already established for `filenameStep`: a file-picker button, a status line, and a single always-enabled Continue (no separate Skip needed -- Continue works with or without a file chosen, same as Output's own optional folder-pick). Reuses `buildAddressMapFromRows()` as-is (the same header-tolerant Account No./Address parser the embedded "ADDRESS" sheet feature already uses) rather than writing new parsing logic -- new `state.manualAddressMap` holds the result. `resolveAddress()` now checks it first, ahead of the shared list and the embedded sheet. `updateAddressLabel()`'s caption reflects whichever source actually won. `collapseAfter()` clears a previously-uploaded file whenever an earlier step changes (new master file, different branch selection), same as it already does for the filename suffix and output folder, so a stale upload can't silently carry over into a different run.

Entirely confined to `tools/branch-split.html` -- no `js/app.js`/publish/encryption involvement, so none of today's Publish-pipeline constraints apply here at all; this is exactly the point of building it this way.

Verified end-to-end via Playwright against a synthetic master file and a fake shared-Address-List bridge (`fake-parent.html`) deliberately overlapping on one account: that account has *both* a shared-list address and a manually-uploaded one -- inspected the generated `.xlsx` with `openpyxl` and confirmed the uploaded address wins, while a second account present only in the shared list correctly falls back to it. Full existing regression suite (7-case flow test, SBA/Reset test, address-fallback/iframe-bridge test) re-run with the new step wired in -- all pass, including that re-uploading the master file / changing branches correctly resets a previously-uploaded address file. `node --check` on both extracted inline scripts clean.

### Fix: Publish can fail with a 422 "not a fast forward" if anything else lands on main at the same moment -- now auto-retries (2026-09-23, same day)

Alok, following the exact upload+Publish steps from the previous round with his real 77,983-row Customer Master file: got "⚠ Publish failed: GitHub API 422 on /repos/mittalok-creator/NPA-DASHBOARD/git/refs/heads/main: Update is not a fast forward. Nothing changed on the live site -- safe to retry." He then asked Claude to publish it directly rather than through his own browser -- declined, since `data/latest.json` is PIN-encrypted and publishing is tied to the Admin's own GitHub identity for a live banking site; investigating the encryption scheme to route around that was (correctly) blocked as credential exploration, and wasn't attempted again. He confirmed this was an expectation mismatch, not a login/PIN failure on his end.

Investigated the actual 422 instead. `publishData()` (`js/publish.js`) reads `main`'s current commit sha up front, builds blobs/a tree/a commit against that sha, then does a fast-forward-only `PATCH` on the ref at the very end -- a multi-step sequence with real GitHub API round-trips in between. If *anything else* lands on `main` in that window -- another Admin publishing, or, confirmed via `git log`/`get_commit` here, a code PR being merged (Claude had merged PR #284 two minutes before Alok's failed attempt) -- the final ref update is rejected with exactly this 422, even though nothing about his own data was wrong. Confirmed via `git log origin/main`: his own retry attempts (`d4e83b3`, `79f9332`, both `mittalok-creator <mittalok@gmail.com>`, both touching `data/latest.json`) had already succeeded by the time this was investigated -- his Customer Master upload IS live. But a non-technical Admin shouldn't have to notice "safe to retry" in an error message and manually retry a multi-step Publish himself, especially when a code deploy landing at the same moment (a real, recurring possibility while this app is under active development) is exactly what can trigger it.

`ghApi()` now attaches `.status`/`.path` to every thrown error. New `publishData()` is a thin wrapper (`rollbackToVersion()`'s own call to it inherits the same protection for free) around the original logic (renamed `publishDataOnce()`) that automatically retries -- re-reading the now-current sha and rebuilding from scratch each time -- up to 3 attempts total, but *only* for this one specific, genuinely-safe-to-retry failure (422 on the `git/refs/heads/...` path specifically). Any other failure -- wrong/expired PIN, an expired GitHub token, a real network failure, a 422 on some other endpoint -- still surfaces immediately on the first attempt rather than being silently retried into a confusing repeated failure.

Verified with a Node reproduction of the exact retry-wrapper logic (copied verbatim) against five scenarios: one race failure then success (2 calls, user sees a "retrying automatically" progress message); two consecutive race failures then success (3 calls, still within budget); a persistent race failure past the 3-attempt budget (correctly gives up and surfaces the error rather than looping forever); a 422 on a *different* endpoint (not retried, fails immediately); and a 401 auth error (not retried, fails immediately). `node --check js/publish.js` clean.

### Fix: real root cause of the recurring "Address List not working" complaint -- uploads were never actually being Published (2026-09-23, same day)

Alok, after Branch Split's Address column was STILL coming up completely empty for every account in the output despite three earlier rounds of matching fixes (Customer ID support, leading-zero normalization + self-healing): "ye address ka kya issue hai kuch aur dekho kaise solve hoga ye to solve hi nahi ho raha kuch changes karne hain to wo dekho" (what's this address issue, look at something else, how will it get solved, it's just not getting solved, look into what else needs changing). Confirmed with him directly (AskUserQuestion) that this time it's the WHOLE output list with no address at all, in Branch Split specifically -- not one account, not the main app's own Loan Detail screen.

Re-investigated from scratch rather than patching the matching logic a fourth time, since `buildAddressListMap()`/`mergeCustomerDetails()`/`resolveAddress()` had all already been directly verified correct (Customer-ID-keyed, leading-zero-normalized) in earlier rounds. Ruled out a load-order race between the main app's async data fetch and Branch Split's `window.parent.UPGB_getAddressList()` bridge -- the full-screen `#dataLoadingOverlay` blocks the entire page, Utility Hub included, until `initApp()` (which is what defines `window.UPGB_getAddressList` in the first place) has already finished running synchronously, so there's no humanly-reachable window where Branch Split could open before the bridge exists.

Found the real cause by re-reading every "applies immediately" upload handler (Address List, Customer Master, Branch Advance, Interest Reversal, Lok Adalat, Special Notes) side by side: every one of them mutates `DATA` in memory and shows a "✔ ... parsed" success message, but **not one of them ever told the user Publish was still required** -- Address List's own message read "Applied ... immediately, and available to Branch Split," which reads as "done" to a non-technical user. `DATA.addressList` living only in that one browser tab's memory until an explicit "Publish to Live Site" click is the exact same failure shape as the KCC Overdue data-loss bug found weeks ago (see that bug's own note on `pendingUnpublishedLabel()`) -- except this time nothing ever warned about it, because these five uploads were never wired into that bug's own protection in the first place. The most likely real sequence: Alok uploaded the Address List, saw the reassuring "applied immediately" message, reasonably believed he was done, and moved on without clicking Publish -- so `data/latest.json` (what Branch Split's bridge, and every fresh page load anywhere, actually reflects) never received it.

**Two-pronged fix, extending the exact mechanism the KCC Overdue bug already established rather than inventing a new one:**
1. New `__hasUnpublishedRefData` flag, set whenever Address List, Customer Master (standalone upload), Branch Advance, Branch Contacts, Interest Reversal, Lok Adalat (upload or Clear List), or a Special Note (save/remove) changes `DATA` in memory -- and folded into the existing `pendingUnpublishedLabel()` the `beforeunload` guard already reads, so closing the tab or hitting Refresh before Publishing now warns for these five uploads too, not just the daily NPA file/PNPA/KCC Overdue.
2. New persistent banner (`#unpublishedRefDataBanner`, styled like the existing warning callouts) sitting directly above the "Publish to Live Site" button, shown/hidden by a new `updateUnpublishedBanner()` -- called from `clearStalePublishStatus()`, which every one of these upload handlers already calls, so it stays in sync for free. Unlike the per-panel "✔ ... parsed" message (which scrolls out of view the moment another panel is touched) or the browser's own dismissible "leave site?" popup, this stays visible and says plainly "⚠ Not live yet — ... Click 'Publish to Live Site' below" for as long as anything is unpublished. Every affected upload's own success message was also reworded to say the same thing explicitly, rather than relying on the banner alone.

No change to any matching/lookup logic at all -- `buildAddressListMap()`/`mergeCustomerDetails()`/`resolveAddress()` are untouched, confirmed still correct in this round's own investigation. Verified the new banner/guard logic with a Node reproduction of `pendingUnpublishedLabel()`/`updateUnpublishedBanner()`/`clearStalePublishStatus()` (copied verbatim, not paraphrased) against a fake DOM: banner stays hidden with nothing pending, shows with the right wording the instant a reference-data flag is set, correctly clears on a successful Publish, and the pre-existing daily-NPA-file case still works unchanged. `node --check js/app.js` clean. Alok will need to re-upload the Address List once this ships and this time follow through to "Publish to Live Site" -- the new banner is there specifically so he can see he hasn't yet.
...
Investigated `buildCustomerMasterMap()`/`mergeCustomerDetails()` directly rather than assuming his read was right: both have only ever matched by Customer ID -- neither reads or needs an Account No. column, so that wasn't the actual cause (confirmed with a standalone Node reproduction of the exact merge against synthetic rows before touching production code). The real cause: `handleMasterFileUpload()` only ever merged the freshly-parsed Customer Master into `__pendingData` -- the in-memory row set from a **daily NPA file uploaded in the same session** -- and `__pendingData` is `null` until that daily file is uploaded. Uploading Customer Master **on its own**, without also uploading a new daily file in the same sitting, silently did nothing: no merge happened, "Apply Update" (which only ever enables from a fresh daily-file upload, never from Customer Master alone) had nothing to enable. Branch Advance, Interest Reversal, and Address List -- the app's other three periodic/standalone reference-data uploads -- all apply immediately on their own upload with no such gate; Customer Master was the one upload that never got the same treatment when it was originally built.

Fixed by giving Customer Master the same "applies immediately" treatment as its three siblings: when no daily file is pending this session, `handleMasterFileUpload()` now merges straight into the **currently live** `DATA.npa.rows` via the exact same `mergeCustomerDetails()` call already used for the daily-upload path, then clears any stale Publish status and re-enables Publish -- exactly Address List's own pattern. The daily-file-pending branch (uploading Customer Master alongside a fresh daily file in the same session) is unchanged. No matching logic changed at all -- Customer Master was, and remains, 100% Customer-ID-keyed; the fix is purely that a standalone refresh can now actually take effect.

Verified the merge itself with a Node-only reproduction (not the full browser flow, since Settings/Update Data sits behind Admin login) mirroring `buildCustomerMasterMap()`/`mergeCustomerDetails()` verbatim: a Customer-ID-only master file correctly fills a blank address/mobile/aadhar for one customer and overwrites a stale address for another, while a customer absent from both the new master and any carry-forward data stays blank -- all by Customer ID alone, Account No. never entering into it. `node --check js/app.js` clean.

### Follow-up: Simple Split's Account No. detection widened, zero-balance rows skipped in both modes, and a typed file-name step replaces the guessed "as on <date>" name (2026-09-23, same day)

Alok, still on Simple Split's formatting: "account no is not in no format and where is date colum set them in date format, where ever is amount and limit word is used set them in no and in 2 point decimal value, and skip those account list where balance amount is 0 in both conversion simple and full. and ek kaam karo ask for the file name but keep branch name and 1 space like maine app main likha NPA AS ON 30-08-2025, afetr conversion name should be like BAMNAI NPA AS ON 30-08-2025 IN BOTH CONVERSION." Four asks: Account No still wasn't landing in number format despite the previous round's fix, skip zero-balance accounts in both Simple and Full output, and replace the guessed `<Branch> as on <date>` filename with one built from a filename he types once per run.

**Account No detection widened**: `classifySimpleColumn()`'s `id` check only matched the literal substrings `accountno`/`acctno` -- real headers like "A/C No" (`acno`), "Acc No" (`accno`), or even "Account Number" (`accountnumber`, which doesn't contain `accountno` as a substring at all -- `t` is followed by `n` then `u`, not `o`) all fell through to plain `text`. Replaced with a broader rule: any header containing `ac` together with `no` or `num` is treated as an id column (plus the existing `sol`/`customerid`/`custid`/`mobileno` checks, widened to also catch `customerno`/`custno`), while the `date` check still runs first so a header like "Account NPA Date" is never caught by the broader `ac`+`no` rule.

**Zero-balance rows skipped in both modes**: Full Split already did this (`buildNpaRows()`'s `bal>0` filter). Simple Split had no such filter -- added one, but since Simple mode has no fixed schema, the balance column can't be found via `idx('balanceamount')` (which only matches Full Split's own exact known header text); new `findColByHeaderSubstring()` finds the first column whose header merely *contains* "balance", same header-guessing spirit as `classifySimpleColumn()`. Only filters when such a column is actually found, so an arbitrary Simple Split file with nothing balance-like in it is left untouched.

**New "File name" step**: a new `filenameStep` panel, inserted between Branches and Output in the existing step-sequencing flow (`STEP_ORDER`, `collapseAfter`/`markDone`/`reopen`), asks once per run for what to type after the branch name -- e.g. typing "NPA AS ON 30-08-2025" against branch "Bamnai" produces `Bamnai NPA AS ON 30-08-2025.xlsx`, identically in both Simple and Full Split (Full Split's filename previously hardcoded an extra "NPA" word of its own; that's gone now since "NPA" is part of what Alok types, not something to bake in). A live preview updates as he types, and Continue is blocked with an inline warning on an empty value. Changing anything upstream (re-uploading, changing branches) clears the typed suffix via the same `collapseAfter()` staleness handling the output-folder selection already had, so a stale filename can't silently carry over to a different run.

Verified with a synthetic file built to exercise all three changes together (an "A/C No" header, one zero-balance row, one blank-balance row, two branches) run through the real Playwright + `openpyxl` pipeline: both output files are literally named `Bamnai NPA AS ON 30-08-2025.xlsx` / `Sikandra Rao NPA AS ON 30-08-2025.xlsx`; the zero-balance and blank-balance rows are absent from both; "A/C No" lands as a real right-aligned number. Full existing regression suite (7-case flow test, SBA/Reset test, address-fallback/iframe-bridge test) re-run with the new filename step wired in -- all pass, including that re-uploading correctly resets the typed filename alongside the output folder. `node --check` on the extracted inline script clean.

### Fix: Address List matching still failed on a leading-zero ID mismatch -- self-healing added (2026-09-23, same day)

Alok, right after the Customer ID fix shipped: "address abhi bhi nahi a para plz fix this issue this is the big issue for me." Investigated rather than guessing again: `buildAddressListMap()` and every lookup site already handled scientific notation, but neither side handled a **leading zero** -- a Customer ID (or Account No.) typed as text in one file ("04521178") and parsed as a genuine number in the other (4521178, Excel silently drops the leading zero on numeric parse) never matched, even though both plainly mean the same customer. Confirmed by building a Node reproduction of the exact scenario before touching any production code: `String(4521178)` = `"4521178"`, the address list's own stored key stayed `"04521178"` -- two different strings, so `map["4521178"]` never finds `map["04521178"]`.

New `normId()` (in both `js/app.js` and `tools/branch-split.html`, mirroring each file's existing duplicated-helpers convention) replaces the ad hoc scientific-notation-only handling everywhere an Account No./Customer ID is used as a matching key -- `buildAddressListMap()`'s own key-building, `mergeCustomerDetails()`/`applyAddressListFallback()`'s lookups, and Branch Split's `resolveAddress()` (its old `normAcct()` renamed, since it's no longer Account-No.-specific). `normId()` never touches what's actually *stored or displayed* elsewhere in the app (a real, deliberate leading zero someone typed still shows on screen exactly as typed) -- it exists purely as a matching-key transform.

**Also self-heals data already published under the old code**: since Alok's Address List upload(s) so far happened before `normId()` existed, whatever's sitting in the currently-published `data/latest.json` still has un-normalized keys -- re-uploading wouldn't even be necessary for this fix to take effect. New `renormalizeIdMap()` re-keys `DATA.addressList`/`DATA.addressListByCustomer` through `normId()` once, right after they're loaded from the published data on every page load (not just at upload time) -- so a stale leading-zero key gets fixed automatically the moment this code deploys. Branch Split's own `loadAddressSource()` does the identical re-keying pass independently on whatever it fetches over the parent-app bridge, so its matching is correct on its own even if a stale parent tab or older code somewhere else in the chain hasn't normalized yet -- defense in depth, not a single point of reliance.

Verified end-to-end, not just the unit-level `normId()` logic: a small fake-parent test harness (a page embedding the real `tools/branch-split.html` in an iframe, serving both bridge functions itself) with a deliberately unnormalized leading-zero key ("04521178") in the customer-keyed map and the matching NPA row's Customer ID as a plain number (4521178, no leading zero) -- inspected the actual generated `.xlsx` with `openpyxl`: address now resolves correctly, alongside the three other cases (account-matched, non-numeric-ID customer-matched, no-match-anywhere) all still resolving exactly as before. Full existing regression suite (7 flow cases, SBA/Reset, Simple Split formatting) re-run with no change in behavior. `node --check js/app.js` and the extracted inline script both clean.

### Fix: Simple Split now formats columns by kind, not raw pass-through (2026-09-23, same day)

Alok: "simple conversion structured nahi lag raha account no no format main nahi hai date no format main hai align nahi hai kuch bhi, no's decimal k 2 points tak and right align nahai hain, name column short hai, yahan list koi bhi ho sakti hai is liye ek set format batana mushkil hoga" -- Simple Split's output didn't look structured: Account No wasn't in number format, dates showed as raw numbers, nothing was aligned, amount columns weren't standardized to 2 decimal places and right-aligned, and the Name column was too narrow. He was explicit that the source file has no fixed schema (unlike Full Split's own known 17 columns), so this can only ever be a best-effort guess at each column's *kind* from its header text, not a hard-coded layout -- exactly the brief.

Root cause: Simple Split copied every column straight through with zero formatting (`row.getCell(c+1).value = r[c]`) -- an account number typed as text in the source stayed left-aligned unformatted text, a date parsed as a raw Excel serial (`XLSX.read(..., {raw:true})`) showed as a plain 5-digit number instead of a date, amount columns kept whatever decimal precision the source happened to have, and every column got the same fixed 16-width regardless of what it held.

New `classifySimpleColumn()` guesses each column's kind from its header text alone (`date`, `id` -- Sol/Account No/Customer ID/Mobile No, `amount` -- anything with amount/amt/balance/limit/outstanding in the name, `name`, `address`, else plain `text`) and applies formatting per kind while writing each row: dates through the same `setDateCell()` helper the Full engine already uses (`dd-mm-yyyy`, only when the value is genuinely a number); id/amount columns through `toNumOrRaw()` (same helper added for the SBA fix two rounds ago) to convert a text-typed number into a real one wherever it genuinely parses, with `id` getting `numFmt:'0'` and `amount` getting `numFmt:'0.00'`, both explicitly right-aligned regardless of whether the underlying value ended up numeric or not. `simpleColumnWidth()` widens Name (28) and Address (40, matching Full Split's own ADDRESS column width) and gives dates a touch more room (14) instead of the previous flat 16 for every column.

Verified against a synthetic file built to specifically exercise every failure mode at once (Account No stored as text in one row and as a real number in another, an amount with a decimal in one row and none in another, a real Excel date, a long name) -- inspected the actual generated `.xlsx` with `openpyxl`: Account No/Mobile No land as real numbers with `numFmt:'0'` and right alignment in both the text-sourced and number-sourced rows; Balance Amount shows `numFmt:'0.00'` and right alignment regardless of the source row's original precision; the date column comes through as a real `datetime` cell with `numFmt:'dd-mm-yyyy'`; Customer Name/Address columns are 28/40 wide. Full existing regression suite (all 7 cases) and the earlier SBA/Reset test re-run with no change in behavior. `node --check` on the extracted inline script clean.

### Fix: Address List now also matches by Customer ID, not Account No. only (2026-09-23)

Alok: "maine address list seprate upload kar di hai usmain cust id di hai ... but split main wo fetch nahi ho rahe" (I've uploaded the Address List separately, I gave Customer ID in it, but Branch Split isn't fetching them) -- and, on follow-up, that the Publish button wasn't responding to clicks either.

Investigated by reading the actual upload code rather than guessing, then confirmed with Alok directly (two questions) rather than assuming: his file has Customer ID only, no Account Number column at all, and he never got Publish to do anything. Root cause, found in `buildAddressListMap()` (`js/app.js`): the Address List upload has *always* required an Account Number column and thrown `Could not find an "Account Number" column` if a file didn't have one -- so his upload never actually succeeded. That explains both symptoms at once: nothing landed in the shared list for Branch Split to fetch, and Publish stayed disabled because it only gets enabled from inside a successful upload handler (unrelated bug, or rather no bug at all -- just the visible symptom of the real one). Alok's real source list is naturally one address per *customer* (an account is opened by a customer who already has an address on file), not one row per account -- a real, sensible shape this feature simply never supported.

**Both identifier types are now accepted**, together or separately, row by row: `buildAddressListMap()` returns two maps (`byAccount`, `byCustomer`) instead of one, keeping Account No. as the more precise match wherever a row has both. New `DATA.addressListByCustomer` (Customer ID -> Address) sits alongside the existing `DATA.addressList`, following the exact same "own slow-moving schedule, part of the publish payload" treatment. The upload's own header-row scan also gained `customerid`/`custid` hints -- a Customer-ID-only file couldn't even find its own header row before, a step earlier than the column-matching error itself. `mergeCustomerDetails()`'s and `applyAddressListFallback()`'s fallback now check Account No. first, then Customer ID. New `window.UPGB_getAddressListByCustomer()` bridge, mirroring the existing Account No. one, so Branch Split can do the same two-step match -- `resolveAddress()` there now takes an optional Customer ID and falls back to it exactly the same way. The downloadable template (`downloadAddressListTemplate()`) now shows both column types with one example row each, and Settings' own panel copy/hints were updated to describe both, so this doesn't quietly break for the next person who assumes Account Number is the only option.

Verified: extracted `buildAddressListMap()` and unit-tested it in Node directly (Customer-ID-only, Account-only, mixed, and no-valid-column-at-all cases) -- confirmed a Customer-ID-only file, which used to throw, now builds a correct map. End-to-end through the real `tools/branch-split.html` via a small fake-parent test harness serving both `UPGB_getAddressList()` and `UPGB_getAddressListByCustomer()`: an account present only in the customer-keyed map, one present only in the account-keyed map, and one in neither, all resolved exactly as expected in the actual generated `.xlsx` (checked with `openpyxl`, not just visually) -- account match wins when both exist, customer match fills in when it's the only one available, blank when neither has it. Full existing regression suite re-run with no change in behavior. `node --check js/app.js` clean.

### Follow-up: Branch Split -- SBA Account/Balance now real numbers, SBA total amount added to Summary, Reset button (2026-09-22, same day)

Alok: "full npa split main ek to sba account and amount ko no. mainn convert kar do, summary sheet main sba balance ka total amount bhi show karo, fir ye complete ho jayega for simple sheet main changes karenge" (in Full NPA Split, convert SBA account and amount to numbers, and show the total SBA balance amount in the Summary sheet too -- then this is complete; Simple Split gets its own changes separately, later). Mid-round he also asked for a Reset button "for next split."

**SBA Account/Balance as real numbers**: both were split out of one `"account->balance"` text cell and written straight through as text, so Excel treated them as text even though `SBA Account`'s column already carried a cosmetic `numFmt:'0'` -- a numFmt on a text-typed cell doesn't make Excel treat it as an actual number (no right-alignment, not usable in SUM/formulas). New `toNumOrRaw()` helper converts each to a real number when it genuinely is one (also expanding scientific notation first, same as the Account No. column already does) and falls back to the original text untouched if it isn't -- never silently blanks a value that didn't parse. `writeNpaSheetBody()` now only applies `numFmt` when the value actually is a number, so a genuinely non-numeric SBA field stays plain text with no misleading number formatting. This is shared by every sheet the Full Split writes (NPA_LIST, category sheets, KCC, both slab sheets), fixed once.

**SUMMARY sheet's SBA block gets a total-amount column**: the "SB BALANCE IN NPA ACCOUNTS" block only ever showed a count of accounts carrying an SBA balance (column F). Added column G ("Amt.(in Lakhs)", same header styling and Lakhs-rounding convention as the main NPA amount column D) with the actual sum, per category and as a Grand Total.

**New "Reset for next split" button** in the Run panel, alongside Run/Pause/Cancel -- reloads the tool's own iframe document rather than hand-resetting every field in `state` and the DOM one at a time, which is simpler and can't miss a field a future change adds.

Verified against the real file: ran a full split and inspected the actual generated `.xlsx` with `openpyxl` (not just visually) -- SBA Account/Balance land as real `int`/`float` cells with the correct `numFmt`, the Summary sheet's new G column shows `0.03` (2,500 / 100,000, correctly rounded) against the one branch/category that has an SBA balance, and the Grand Total row matches. Reset button reloads cleanly back to the untouched Step 1 state (confirmed: `fileStep` no longer `done`, every later step re-hidden, the drop zone's label back to its default text). `node --check` on the extracted inline script clean.

### Redesign: Branch Split -- upload-first step-by-step flow, polished controls, silent address auto-fetch (2026-09-22, same day)

Alok asked (Hinglish): "pahle file upload karao fir option do select karne k... uske baad jo bhi options and buttons hain wo bhi redesign karo... address agar backend par uploaded hai to yahan se mat karao apne aap fetch karo" -- upload should be the genuine first step, everything else should reveal one at a time after it (not the old three-chunk reveal), every control should get a visual pass, and the address list must never require action from the user in this tool. Confirmed 4 decisions up front (AskUserQuestion): step-by-step reveal over one combined options panel; keep this tool's own existing green accent identity rather than reskinning to the Utility Hub's brass look; remove the address override entirely, not just soften it; and preview-first via a Claude Artifact mockup (two states -- mid-flow and everything-collapsed-ready-to-run) -- approved as-is ("Ok done") before any code changed.

**New step-sequencing mechanism** replacing the old scattered inline `hidden` toggles: each step is a `.panel` that is either not-yet-reached (`[hidden]`, unchanged), open (the step being worked on), or collapsed to a compact "✓ value ... Change" summary row (a new `done` class). `STEP_ORDER` plus `collapseAfter()`/`markDone()`/`reopen()` drive the whole sequence -- Master file, then Worksheet, then Mode each collapse the moment they're individually confirmed; Region (skipped entirely if the sheet has no Region column) and Branches reveal together and collapse together via a new "Continue" button, since both already carry working auto-defaults and gating them one-at-a-time would only add clicks with no real decision behind them; Output collapses the instant a folder is picked, or -- in a browser without folder-picker support -- auto-collapses immediately with "Will download as one ZIP file" since there's nothing left to decide. Every "Change" link reopens that step and correctly resets everything after it (including a previously-picked output folder, now also reset on a fresh file upload -- it never was before).

**Real bug fixed along the way, not just relocated**: the Full NPA Split mode's required-column check (`REQUIRED_FULL`) set a warning message but never actually blocked anything -- a user could reach Run in Full mode with columns missing. Since Mode now comes after the sheet is already parsed, this became `validateModeColumns()`, called from the Mode buttons themselves: a real gate now, re-checked every time Mode is clicked, using the already-parsed header map (no re-parsing needed).

**Address list step removed entirely**, per Alok's explicit choice: no panel, no "CHANGE (OPTIONAL)" button, no upload input -- the shared Address List (fetched live from the main app) is used automatically if present, falling back to an "ADDRESS" sheet inside the uploaded file itself, else left blank, exactly as before. The only remaining trace is a small read-only caption above the Run button reusing the existing resolution logic's own wording, colored amber as a warning only when neither source has anything to offer.

Verified via Playwright against the real file (not a copy): the full existing regression set (small synthetic master, title-row file, no-Region file, deliberately-no-Branch file, real small sample) all still load and flow correctly; a new deliberately-columns-missing file confirms the Full-mode gate now actually blocks (Region/Branch never appear) and Simple mode on the same file passes straight through; unchecking a branch and later changing Mode via "Change" preserves that selection (confirmed: "2 of 3 selected" survives); re-uploading a different file correctly resets every downstream step including a previously-picked output folder; a full end-to-end run (ZIP-fallback path, forced by removing `showDirectoryPicker` since a native picker can't be automated) produced two correctly-named `.xlsx` files and skipped a zero-row branch exactly as the unchanged business-logic engines have always done. `node --check` on the extracted inline script clean; both themes and the mid-flow/hover states screenshotted, matching the approved mockup.

### Redesign: Utility Hub -- "toolkit" identity, grouped into categories, with a live search box (2026-09-22, same day)

Alok asked for the Utility Hub panel to be redesigned: "kuch elegant kuch refreshing" (something elegant, something refreshing). Investigated the existing implementation first, then built and published a Claude Artifact mockup (real tool names/descriptions/icons, both dark and light themes) before touching any code -- Alok reviewed and approved it as-is ("Done live kar do") before this round shipped.

Three decisions confirmed with Alok up front (AskUserQuestion): a full panel redesign, not just card polish -- also grouping and search; a visual identity deliberately distinct from the Dashboard's blue/glass hero cards, not a copy of it; and preview-first, mirroring how the Recovery Dashboard was shipped on 2026-09-19 ("pahle bana k dikhana, fir live karenge") and how the original embossed-card look itself was approved from a mockup on 2026-09-10.

**Grouped the 8 tools into 3 categories** by what they actually do -- Converters (PassSheet, HBR Report, Branch Split), Summaries & Reports (NPA Region Summary, Daily PNPA Summary, NPA SOL Summary), Directory & Reference (Telephone Directory, Branch Map) -- each with its own heading, a colored rule, and a tool-count pill. **Added a live search box** above the grid (`utilityFilterInput()`, new in `js/app.js`): pure DOM show/hide keyed off each card's own title+desc text, no data array or state duplicating what the markup already has, so a category with zero matches hides itself too and an empty-state message appears when nothing matches at all.

**New visual identity**, deliberately not a copy of the Dashboard's blue glass cards: round "coin" icon badges (a radial gradient of each tool's own hue rather than a flat soft-fill square), a brass/copper accent thread for the header icon and category headings -- reusing the theme-independent `--seal`/`--seal-l`/`--seal-d` tokens that already give the OTS Calculator its own "instrument" identity, so Utility Hub reads as a sibling instrument rather than a fourth accent language invented from scratch -- and a faint dot-grid ground texture in place of the Dashboard's blurred color blobs. Each `.utility-card` now carries its own `tool-<hue>` class directly (moved from the icon span it used to sit on) so the card's own hover border can pick up the tool's exact color via CSS custom-property inheritance, instead of every card hovering into one identical accent the way the old design did.

Verified via Playwright against the real markup/CSS/JS (not a copy) at both the DOM and pixel level: 8 cards across 3 categories render correctly; searching "branch" correctly surfaces every card whose title *or* description contains it (5 of 8 -- several descriptions genuinely say "branch-wise"), a narrower query ("GPS") correctly narrows to exactly one; the empty-state message appears and clears correctly; all 8 cards still fire their original `switchView()` call with the exact same view name as before (navigation wiring untouched); both themes and the hover state screenshotted and match the approved mockup. `node --check js/app.js` clean.

### Fix: Branch Split's real root cause -- a 10-million-cell SheetJS parsing ceiling, not a header-detection problem (2026-09-22, same day)

Two rounds of header-detection fixes above didn't fix it: Alok kept hitting "Could not find a 'Branch'... column" against his real `e-AB NPA AC WISE` daily export. The diagnostic message added in the previous round was the key -- it showed "Row 1 as read" was a literal empty array `[]`, not a wrong header row. Alok also gave two precise clues: a file scoped to one region loads fine, the same export with multiple regions (his real full daily file) doesn't; and an exact row count for the failing file, 740,000 rows.

Investigated by reproducing directly rather than guessing a third time. Generated a real 200,000-row x 35-column `.xlsb` (114MB, spanning 4 regions) using this app's own vendored SheetJS build inside an actual Chromium tab (Node itself can't run `XLSX.writeFile`'s browser APIs, which cost an earlier attempt real time before being recognised as unrelated) -- it loaded correctly through the live tool, ruling out "multiple regions" and raw file size (114MB) as the cause on their own. 200,000 rows x 35 columns = 7,000,000 cells; 740,000 rows x 35 columns = 25,900,000 cells. Searching SheetJS's own documentation and issue tracker turned up a well-documented practical ceiling on its default ("sparse") parsing mode, around 10 million cells, past which large sheets silently return empty data in Chrome rather than throwing a catchable error -- matching the empty-array symptom and the "works for one region, fails once enough regions push it past ~10M cells" pattern exactly. The documented fix: pass `dense:true` to `XLSX.read()`, switching to an array-of-arrays internal representation documented to scale to roughly 50 million cells -- comfortably past Alok's 25.9M -- and documented to work transparently with `XLSX.utils.sheet_to_json()`, already used everywhere in this tool, so no other code needed to change.

Added `dense:true` to both `XLSX.read()` call sites in `tools/branch-split.html` (the master-file parser, and the address-override upload parser) -- a pure capability increase, not a behaviour change, for every file that already worked. Verified with no regression across the full existing regression set: the original small synthetic master, the title-row file, the no-Region file, the deliberately-no-Branch-column file, Alok's own small real sample, and -- reusing the still-available 200,000-row/114MB real `.xlsb` from the investigation above -- that file too, all loading exactly as before.

Honest caveat: a full round-trip at Alok's actual 740,000-row scale could not be completed in this environment (each attempt at generating a file that size hit unrelated environment limits -- a container restart, a script-design flaw with a blocking browser call). This fix is shipped on the strength of the documentation match plus the successful 200K-row/7M-cell repro (just under the known-safe range), not a confirmed 740K-row test -- Alok, please retry your real file directly; if it still fails, the diagnostic message already live will show real content instead of `[]`, or a different signature entirely, either of which narrows the next step immediately.

### Follow-up: Branch Split's header scan widened + made diagnosable (2026-09-22, same day)

The 10-row header scan from the fix above still didn't find "Branch" on Alok's actual file -- it's a real ~180MB `.xlsb` (vs. the tiny samples used to verify the first fix), and a workbook that size almost certainly carries far more padding/formatting bloat above its real data than 10 rows' worth. Couldn't get the real file to test directly (well over the 30MB upload limit here); a smaller sample he provided (`npa_21-9-26.xlsx`, real HO layout, header at row 1) parsed correctly both in a standalone reproduction and through the live tool, which didn't explain his continued failure -- so the fix widens `findHeaderRowIndex()`'s scan from 10 to 200 rows (still cheap, in-memory only) rather than guessing further, and turns the failure message itself into a diagnostic: it now shows exactly what row 1 contained (or, if a header row *was* found but had no Branch column, exactly which columns it did find) -- a screenshot of the error is now enough to debug the next mismatch without needing the file at all.

### Fix: Branch Split couldn't find the "Branch" column on a real HO export (2026-09-22, same day)

Alok tried the new tool against a real file (`e-AB NPA AC WISE 21.09.2026.xlsb`) and hit "This sheet has no 'Branch' column -- pick a different sheet", even though the screenshot he sent back clearly shows a Branch column right there in the sheet. Root cause: `loadSheet()` assumed the uploaded sheet's very first row was always the header row and read column names straight off it -- any report-title/blank row above the real header (common in hand-prepared or re-saved exports) meant every header name came back wrong, "Branch" included. The main app's own Customer Master/Branch Advance/Address List uploads solved this exact problem days ago with `findHeaderRowIndex()` (scans the first 10 rows for one that actually contains a recognizable header) -- this tool's own file-reading code just never got the same treatment when it was built earlier today.

Ported the same `findHeaderRowIndex()` helper into `tools/branch-split.html` and used it everywhere a header row is assumed: the main master-file sheet parser, the one-off address-override upload, and the embedded "ADDRESS" sheet fallback. Verified with two synthetic files -- the original header-at-row-0 file still loads correctly (no regression), and a new file with two title rows above the real header now loads correctly too (previously would have failed the same way Alok's did).

### New Utility Hub tool: Branch Split, plus a new app-wide Address List (2026-09-22, same day)

Alok uploaded two things he'd built to solve the same recurring job -- splitting the daily NPA export into one file per branch -- and asked whether they should become one utility: a simple `.xlsb` macro (`Split_Data_in_workbooks`, one flat sheet per branch, hand-typed folder path/title every run) and `BranchSplitConsole.ps1`, a WinForms desktop app he'd built next, the mature version of the same idea (Region-aware, address-enriched, writes a multi-sheet workbook per branch -- NPA_LIST/SUMMARY/category sheets/KCC/₹5-10L & ₹10L+ slabs -- already validated against real 21-Sep-2026 data). Investigated both (extracted the macro's VBA and read the full `.ps1` script) before answering: they aren't two tools to merge -- the console app is the fully-matured version of the exact idea the macro started, and its own dormant "plain split" engine already covers the macro's behaviour. Confirmed the target and two open design questions with Alok directly (three rounds of AskUserQuestion): both modes in one new web tool (Simple Split / Full NPA Split, address enrichment in both), Chrome/Edge gets a true native folder-picker with a ZIP-download fallback elsewhere, and -- Alok's own words -- "1 list main upload karun and entire app main wo use ho jahan bhi address available nahi hai": the address list needed to be genuinely app-wide, not scoped to one utility.

**New `DATA.addressList`** (Account No. -> Address), following the exact `DATA.interestReversalMaster` pattern from two days ago: new admin-gated "Address List" panel in Settings -> Update Data, applies immediately, persists through Publish (`js/app.js`: init, `buildAddressListMap`, `handleAddressListUpload`, `downloadAddressListTemplate`, `dataObj` publish payload). The actual fallback is one line inside `mergeCustomerDetails()`'s existing loop -- if an account's address is still blank after Customer Master + carry-forward, `DATA.addressList` is checked last, by Account No., only ever filling a blank rather than overwriting a real address -- so it covers every place that function is already called, present and future, without a second call site to remember.

**New `window.UPGB_getAddressList()`** -- a deliberate, one-off exception to "every Utility Hub tool is fully independent": since the address list must be genuinely single-sourced across the main app and the new tool, `tools/branch-split.html` reads it live off the parent app (same-origin, plain `window.parent` property access, no postMessage) instead of keeping its own private copy, degrading gracefully to "no shared list" if opened outside the app's iframe.

**New `tools/branch-split.html`** -- a standalone Utility Hub tool (own `<style>`/`<script>`, ExcelJS vendored, ported logic from `Run-NPASplit`/`Run-GenericSplit`), added to the grid with a new `tool-gold` icon hue (`css/styles.css`, the first new Utility Hub icon color since the original five): Mode toggle (Simple/Full, Full default) -> master file upload -> worksheet picker -> a Region step that only appears when the sheet actually has a Region column (defaults to HATHRAS, exactly matching the console app's own skip-when-absent behaviour) -> branch checklist -> address source (shared list, or a one-off per-run override upload) -> output (native folder-write when `showDirectoryPicker` is available, else one ZIP via a newly-vendored `js/vendor/jszip.min.js`) -> Run, with Pause/Resume/Cancel checked between branches (never mid-file) and an activity log, mirroring the console app's own run semantics. Full NPA Split reproduces `Run-NPASplit` exactly: `Balance Amount>0` filter, the 17-column `NPA_LIST` (styled/frozen/autofiltered), `SUMMARY` (category counts/amounts + SB-balance count), one sheet per asset category actually present, `KCC` (scheme CC004), and `NPA_5L_to_10L`/`NPA_Above_10L` bucketed by each **customer's total balance across their accounts in that branch**, not per-row -- a branch with zero qualifying rows is skipped entirely (no empty file), logged either way. Simple Split is the macro-equivalent flat copy, with the one deliberate addition Alok asked for: address enrichment in both modes, not just Full.

Verified with synthetic data end-to-end (not just visually): a test master workbook (mixed asset categories, a customer whose 2 accounts cross the 5L/10L bracket boundary, an `SBA Acc/Balance` cell, a Region column spanning two regions, a zero-balance branch, a scientific-notation account number) plus a synthetic Address List, run through both engines. Caught and fixed a real bug along the way -- Full Split's `NPA_LIST`/category sheets were writing a scientific-notation account number straight through unexpanded (e.g. `1.51635110000110E+14`) instead of the plain digit string the main app's own daily upload always produces; fixed to match. Confirmed: `SUMMARY`'s category counts/amounts reconciled exactly against the synthetic data by hand; category/KCC/slab sheets appeared only when non-empty; the zero-balance branch produced no file but was logged; the Region step showed/defaulted/hid correctly with and without a Region column; the address bridge correctly reported the exact same record count as the main app's own upload, both standalone and via the shared list; the app-wide fallback filled a real account's address with no Customer Master uploaded at all, verified in the Loan Detail screen. `node --check js/app.js` clean, both new HTML files well-formed.

### Fix: Interest Reversal now sourced permanently from Alok's own list, never silently resets again (2026-09-22)

Alok asked: "interest reversal ki jo excel maine di thi kya wo ab update nahi hai app main?" Investigated by decrypting the live production `data/latest.json` directly (not guessed): all 13,535 accounts showed Interest Reversal as blank/0 -- including account `161573210000110`, which the 2026-09-08 entry below documents as showing `5525` right after Alok's `Interest_Reversal.xlsx` was first merged in.

Root cause, confirmed by reading the actual parsing code: the daily NPA upload (`mapHoRowsToNpa()`) reads Interest Reversal fresh from Head Office's own file each day (its "Intt Rev" column). Unlike Address/Mobile/Aadhar/PAN -- which carry forward from the previous dataset when HO's file doesn't have them -- Interest Reversal had **no carry-forward at all**. HO's own "Intt Rev" column has evidently been blank or unreliable, so the very next regular daily upload after 09-08 silently reset it back to 0, and every upload since kept it there. Unrelated to the 09-20 rollback above -- this predates it entirely.

This wasn't a one-off data-repair request either: Alok asked for Interest Reversal to come from *his* list every single time he uploads the NPA list, from now on. Checked whether the original file itself was still available to reapply -- it wasn't: this app never persists a raw uploaded Excel server-side, only the resulting merged figures, and a full git history search confirmed no such file was ever committed. Alok then supplied the file directly in this conversation, confirmed to be the same one: `Interest_Reversal.xlsx`, `Account_Number` + `Interest Reversal` columns, 9,821 rows.

Added a new persistent reference field, `DATA.interestReversalMaster` (Account No. -> amount), following the exact same "own slow-moving schedule, persists through Publish, applied automatically without re-uploading" pattern already established for `DATA.branchAdvances`/`branchContacts`/`specialNotes`/`lokAdalat` -- deliberately *not* the Customer Master (`__pendingMaster`) pattern, which turned out not to persist to `DATA`/publish at all and would have hit this exact same silent-reset problem. New admin-gated upload panel "Interest Reversal (master list)" in Settings -> Update Data (`handleInterestReversalMasterUpload`, `buildInterestReversalMasterMap`), applies immediately (no separate Apply step, matching Branch Advance). New `applyInterestReversalMaster()` overrides Interest Reversal by Account No. on every future daily NPA upload -- for any account the master covers, its figure wins over whatever (if anything) HO's file carries that day; accounts the master doesn't cover keep HO's own file's value unchanged. Wired into both the current HO-format upload path and the legacy-workbook path, so neither can drift out of sync.

Verified end-to-end via Playwright against real logic (not just visually): uploaded a synthetic Interest Reversal master (1 account, ₹5,000) alongside a synthetic daily NPA file carrying its own conflicting Interest Reversal figures for 3 test accounts (₹999 for the account in the master, ₹777 and blank for two accounts not in it). After Apply, the OTS Calculator correctly showed **5,000** or the account in the master (override wins over HO's 999), **777** for the account only in HO's file (correct fallback), and blank for the account in neither (unchanged, matching prior behaviour) -- all three cases exactly as designed. `node --check js/app.js` clean.

Alok's real `Interest_Reversal.xlsx` still needs to be uploaded once through the new panel (Settings -> Update Data -> "Interest Reversal (master list)") and Published to actually restore live figures -- this round shipped the mechanism; seeding production data is the next, separate action.
...
Resolved with Alok (two decisions, this session): the **Branch Data Excel is now the single, permanent source** for Gross NPA/Advances/NPA ratio on the Dashboard -- not a dynamic "freshest wins" comparison -- so every other place computing this same trio was removed or relocated to a single home:
- Removed the "Total Outstanding" hero tile entirely (`renderDashboard()`'s hero row is now 3 tiles -- Total Accounts, High-Risk Exposure, Average Ticket Size -- none of which duplicate anything Recovery already shows above).
- Removed the "X% NPA" badge and the Mar/Jun-baseline corner-stats card from the hero row; that same `DATA.branchAdvances`-based computation now lives *only* inside the Recovery band's own fallback state (`renderDashRecoveryFallback()`), so there is exactly one home for "what's the NPA ratio right now" at any moment -- Recovery Excel's number when it's loaded, the Branch Advance file's number as a fallback when it isn't, never both.
- The Top-10-Branches list's per-branch NPA% badge now prefers the Recovery Excel's own ratio for a branch it covers (matched by Sol ID, consistent with the band above), falling back to the old Branch-Advance-based computation only for a branch Recovery doesn't cover.

**"KCC rollover data"** turned out to mean the separate **KCC Overdue** upload/tab, not the Recovery band's own RCT "Rollover %" column or the NPA book's KCC/Non-KCC scheme split (both confirmed unrelated, both untouched) -- clarified with Alok before writing any code, since the codebase actually has three different things that phrase could mean. Rather than guess the exact business rule for bucketing KCC Overdue's rows into "due this month" (unclear which date column, which cutoff -- getting it wrong in a banking dashboard is worse than not showing it), this round adds the live KCC Overdue upload's own current count/OS as a **visible, labeled cross-reference** line inside the Recovery band's "KCC PNPA — this month" pipeline card (region-wide or scoped to the selected branch, each with its own "as on" date) -- not a silent replacement of the Excel-sourced figure. `data/kcc-overdue.json` is now fetched lazily as soon as the Dashboard opens (`ensureKccOverdueDataLoaded()`, mirroring `ensureBranchRecoveryDataLoaded()`), not only when the KCC Overdue tab itself is opened.

Verified against Alok's own real, already-published data (`data/branch-recovery.json`, `data/kcc-overdue.json` both live in this repo as of yesterday and the day before): full Playwright walkthrough showing exactly one Gross NPA figure on screen at all times, the 3-tile account-level hero row, the KCC cross-reference line correctly scoped both region-wide (5,706 a/c, ₹128.73 cr) and to a single branch (Bamnai: 98 a/c, ₹3.34 cr) matching its Sol ID, and the OTS Calculator unaffected (`openDetail` still opens the worksheet identically) -- screenshotted in light and dark.

### New: Recovery Dashboard (Region + Branch), from Alok's own daily Excel workbook (2026-09-19)

Alok already builds a near-daily Excel workbook (`RO_Hathras_Recovery_Dashboard.xlsx`) tracking recovery performance across all 55 branches -- Gross NPA/Advances/ratio, movement since Mar-26, Sep-26/Mar-27 target tracking, forward KCC PNPA/Limit Review pipeline, a top-10 NPA watchlist, reductions/increases leaderboards, ticket-size spread and RCT (Rollover/Collection/Turnover) efficiency -- and wanted the same view live inside the app instead of Excel, updated by uploading fresh branch data frequently.

Shipped in two steps, per his own request ("pahle bana k dikhana, fir live karenge" -- show a preview first, wire it live only after approval):
- **Preview**: a standalone mockup built from his real uploaded workbook, art-directed to look "impressive" rather than a literal Excel-grid copy (hero KPI band, gradient-filled trajectory chart with target lines, radial target-tracking gauges, ranked watchlist bars, color-coded rank badges), published as a reviewable link and approved before anything touched the live app.
- **Live**: a new "Recovery" nav tab with Region and Branch views, built to match the approved preview, restyled onto this app's own existing CSS tokens (`--card`/`--line`/`--accent`/`--pos`/`--neg`/`--amber`/`--radius-*`/`--font-mono`) rather than a parallel palette. New upload panel in Settings → Update Data (`Recovery Dashboard — Branch-wise data`) parses the "Branch Data" sheet client-side (`XLSX.utils.sheet_to_json`, matched by sheet name so either just that sheet or the full 4-sheet workbook works) and computes every rank, the watchlist, the reductions/increases leaderboards and the ratio-spread counts itself from the 55 branch rows + region-total row -- deliberately not dependent on the workbook's own pre-computed DASHBOARD sheet, since Alok's stated habit is uploading only the raw branch-wise sheet each time, not the whole workbook. Ranks replicate the sheet's own `COUNTIF`-based logic exactly (descending = largest/highest first for most metrics, ascending specifically for movement-since-Mar-26 since more negative = bigger reduction = better = rank 1) -- verified against the workbook's own worked example (Bamnai: all 9 ranks matched exactly).

New file `data/branch-recovery.json`, published through the same `extraFiles` + `encryptToEnvelope` mechanism already built for `data/pnpa.json`/`data/kcc-overdue.json` -- encrypted behind the PIN from day one, same as every other sensitive data file in this app. Fetched lazily (only when the Recovery tab is actually opened), same lazy-fetch pattern as Daily PNPA/KCC Overdue.

Verified: the live parser's output matches the earlier-verified extraction exactly (Node test running the actual `js/app.js` parsing code against the real workbook -- region totals, Bamnai's 9 ranks, watchlist/leaderboards/ticket-spread all byte-for-byte identical); full Playwright walkthrough of the real app -- upload the workbook, open Recovery → Region, switch to Branch, click a watchlist row to drill into that branch -- with zero console errors, screenshotted in both Region and Branch views.

### Follow-up: Recovery Dashboard merged into the main Dashboard, one tab instead of two (2026-09-19, same day)

Hours after the standalone "Recovery" tab shipped, Alok asked to merge it with the existing "Dashboard" tab into one improved dashboard: the Branch Data Excel becomes the summary/headline at the top, the account-wise daily NPA upload keeps powering the OTS Calculator exactly as before, and the existing "click a number → see the accounts → open in OTS" linkage stays available. Investigated first (not assumed) whether this was safe: confirmed the OTS Calculator (`runSearch`/`openDetail`/Cmd+K) reads `DATA.npa` exclusively and has zero code-path overlap with `BRANCH_RECOVERY_DATA` anywhere, so a dashboard-layout merge could not touch it.

The standalone Recovery tab (and its own Region/Branch sub-tabs and branch picker) is gone. In its place, `#dashRecoveryBand` sits above the existing account-level Dashboard content, inside the same `data-view="dashboard"` view, and follows the Dashboard's own single `#dashBranchFilter` -- no second branch picker. The two filters used to key differently (old Dashboard by branch *name* string against `DATA.npa` rows, Recovery by *Sol ID* against the Branch Data sheet); unified by reusing `computeDashboardStats()`'s own `branchMap` (which already carries each branch's Sol ID straight off the NPA rows, the same source `dashboardBranchInfoCard` already trusted) to resolve the picked branch name to a Sol ID, then looking that Sol ID up in `BRANCH_RECOVERY_DATA.branches` -- no second name/ID mapping invented, and confirmed working even where the two sources spell a branch differently (e.g. account-wise data's "BAMNAI" vs the Excel's "Bamnai" both resolve to Sol ID 9271 and render the same branch).

When `data/branch-recovery.json` hasn't been uploaded yet (or fails to load), the band falls back to whatever the daily NPA upload can actually provide -- Gross NPA, account count, and a clickable top-10-branches-by-NPA list via `computeDashboardStats()` -- so the Dashboard is never empty waiting on a second upload. Trajectory-over-time, Sep-26/Mar-27 targets, the forward KCC-PNPA/Limit-Review pipeline and RCT efficiency have no equivalent anywhere in the account-wise data, so those cards simply don't render in the fallback state rather than showing fabricated numbers. The watchlist's "jump to this branch" click (`drillBranchBySol`) now drives the one Dashboard filter instead of a separate Recovery branch picker, scoping both the summary band and the account-level detail below it together.

Also fixed in passing: the target-tracking gauge rings' background track referenced an undefined `--card-2` token (should have been `--rec-card-2`, this app's actual token) -- the track silently rendered invisible instead of erroring, only the colored arc showed. Caught by re-reading the code while retargeting these functions, not by a visual regression -- worth a second look since it shipped unnoticed in the original Recovery tab a few hours earlier.

Verified: full Playwright walkthrough of the real merged app -- fresh load with no Recovery upload yet (fallback band + top-branches list, zero Recovery nav items left), upload the workbook via Settings (band switches to the full Region view live, no page reload), switch the one branch filter to a specific branch (band and account-level branch-info card both scope to the same branch despite differing name spellings), click a watchlist row (filter updates to that branch), click an account row (still opens the OTS Calculator worksheet, unchanged) -- zero console errors, screenshotted in light and dark.
...
Fixed by encrypting all three files (AES-256-GCM, key derived via PBKDF2-SHA256/200,000 iterations from the same PIN the splash screen already asks for) -- every viewer's experience is unchanged, same PIN screen, same 4 digits. A wrong or missing PIN can no longer even see the JSON shape, only ciphertext wrapped in a small self-describing envelope (`{"enc":1,"kdf":...,"salt":...,"iv":...,"plainHash":...,"data":...}`) so the raw file still opens as valid JSON rather than looking broken. **Threat model, stated plainly**: this protects against the realistic risk here -- someone stumbling on the public URL or scraping it -- not a determined technical attacker. A 4-digit PIN is only 10,000 combinations; offline-cracking the ciphertext is estimated at roughly 15-20 minutes for someone who deliberately sets out to do it. Alok was told this directly and chose to keep the PIN at 4 digits rather than lengthen it, accepting that tradeoff.

Two real bugs surfaced and fixed during implementation, both worth recording since they're the kind that would only show up in production otherwise:
- **PIN handoff race**: `loadNpaData()` used to fire immediately when `js/app.js` finished executing -- well before a human finishes typing 4 digits into the splash screen -- so it always lost the race and hit a decrypt failure on every fresh session. Fixed with a small event handshake: `js/splash.js`'s `unlock()` now dispatches `upgb-pin-unlocked`, and `js/app.js` only calls `loadNpaData()` immediately if a PIN is already in `sessionStorage` (a same-tab-session return visit skips the splash screen entirely), otherwise it waits for that event.
- **`npaChanged` dedup regression**: `publishData()`'s existing "did the NPA book actually change" check (added 2026-09-09-ish, comparing git blob shas) would have silently broken -- encryption uses a fresh random salt/IV on every publish, so the ciphertext blob sha now differs even when the underlying data is byte-identical, meaning every single publish would look like a real change and spam a new history snapshot. Fixed by storing a `plainHash` (SHA-256 of the plaintext, itself not secret) in the envelope and comparing that instead once the live file is already the new encrypted format.

Also handles the transition cleanly: `rollbackToVersion()` detects and decrypts only the new envelope format, leaving the ~30 existing plaintext history snapshots untouched (not bulk-rewritten) -- rolling one of those back naturally re-encrypts it on the way out, since publishing always encrypts. A decrypt failure (missing/wrong PIN, corrupted data) shows "Could not unlock NPA data — reload and re-enter PIN" with a Reload button that clears the session and forces the splash screen back, rather than the generic network-error retry (which can't fix a bad key).

Verified: a standalone Node round-trip test of the exact crypto (encrypt→decrypt match, wrong-PIN correctly fails, same plaintext→same `plainHash`→different ciphertext); a full Playwright run entering the real PIN against the real re-encrypted repo files end-to-end (data renders, zero console errors, ~150ms encrypt/decrypt even for the full 4MB `data/latest.json`); the decrypt-failure→Reload→re-enter-PIN recovery loop; and `publishData()` against a mocked GitHub API confirming the `plainHash` fix -- identical data published twice correctly produces `npaChanged:false` on the second call (no spurious history snapshot), where the old sha-based check would have said `true` both times.

### Performance: fixed the "slow/blank to open" problem across 4-5 computers on different networks (2026-09-17, same day)

Alok reported the app taking a long time to open and sometimes going fully blank, tried across several branch computers on different networks, and guessed it might be a loading animation. A real audit turned up three genuine causes, none of them animation -- the splash/login screen is cheap CSS/timeout-driven and doesn't gate or block anything:

1. **Every single tool tab loaded its own heavy scripts immediately, not just the one tab being viewed.** The dashboard's 7 tool tabs (PassSheet, Region Summary, PNPA Summary, Telephone Directory, HBR Report, NPA SOL Summary, Branch Map) are each a separate page embedded via `<iframe>` -- and every one of those iframes had a real `src` set directly in `index.html`, so all 7 loaded in full the moment the dashboard itself opened, regardless of which single tab a viewer actually wanted. Several of those tool pages carry their own multi-hundred-KB-to-1MB+ vendor scripts (pdf.js, xlsx, exceljs) -- roughly 3.5MB combined, invisible to anyone just glancing at index.html's own script tags. Fixed by switching every tool iframe to `data-src` instead of `src`; `switchView()` in `js/app.js` now copies `data-src` into `src` the first time (only) a viewer actually opens that specific tab.

2. **The main dashboard's own 5 heavy vendor libraries (msal-browser, xlsx, exceljs, html2canvas, jsPDF -- ~2.76MB combined) were also plain blocking `<script src>` tags on every load**, even though the huge majority of visits are just viewing NPA data and never touch OneDrive login, Excel import/export, or the WhatsApp PDF share button. Removed those 5 tags from `index.html` entirely; a small loader (`loadVendorScript()` + `ensureXLSX()`/`ensureExcelJS()`/`ensureHtml2Canvas()`/`ensureJsPDF()`/`ensureMsal()`, top of `js/app.js`) now injects each one's `<script>` tag on first actual use instead -- every upload handler, the 3 Excel-export functions, both WhatsApp-share functions, and the OneDrive `onedriveMsal()` gate now `await` the relevant one before touching that library's globals.

3. **`data/latest.json` (4MB+) had no fetch timeout, so a merely stalled connection (not an outright failure) could hang the page forever with nothing visibly wrong** -- the existing retry-then-error-with-Retry-button handling only ever fired on a real rejected fetch, never on one that just never resolved. This is likely the direct explanation for "sometimes goes blank": no error, no spinner change, just stuck. `fetchJson()` (shared by the NPA data load, the offline-save button, and the PNPA/KCC Overdue tab loads) now wraps every call in a 30s `AbortController` timeout, generous enough not to abort a genuinely slow-but-progressing transfer.

Also reworked `sw.js`'s caching strategy while touching this: the app shell (HTML/CSS/JS/manifest) moved from pure network-first -- meaning even a repeat visit to an unchanged app had to wait on a live round-trip before anything appeared, worse the weaker the network -- to stale-while-revalidate (serve the cached copy instantly if one exists, refresh it in the background for next time), safe alongside this app's existing versioned-URL cache-busting since a version bump is always a brand-new URL with nothing cached yet. `data/latest.json` deliberately keeps its old network-first-with-cache-fallback behavior (Alok's explicit 2026-09-12 request: never show stale banking figures while genuinely online). Also dropped `xlsx.full.min.js`/`exceljs.min.js` from the precache list, since they're no longer eagerly loaded by anything on the dashboard's own first paint.

Verified locally: a fresh page load now transfers ~5.7MB total, almost entirely the unavoidable 4MB NPA data fetch itself (down from an estimated 12MB+ before, once the ~2.76MB main-page vendor scripts and ~3.5MB of eagerly-loaded tool-tab scripts are both counted) -- confirmed via a network listener that zero of the 5 main vendor libraries and zero tool-iframe pages load until actually used, that a tool tab lazy-loads correctly on first open and doesn't reload on a repeat visit, that a deliberately-hung connection is aborted and surfaces the existing retry/error UI instead of hanging forever, and that with the app shell's own network completely blocked, a repeat page load still renders correctly (title, full DOM) in a few hundred ms, served entirely from cache.
...
Checked each against its real latest version rather than assuming "old = bad":
- **xlsx (SheetJS)**: was 0.18.5, which is actually the newest version npm itself carries -- SheetJS moved later releases to their own CDN after an npm policy dispute, so `npm view xlsx version` misleadingly reports 0.18.5 as "latest." Real current is 0.20.3, fixing known prototype-pollution and ReDoS advisories from the 0.18.x line. Upgraded via `cdn.sheetjs.com` (the vendor's own distribution channel). Every `XLSX.*` call in this app (`read`, `utils.sheet_to_json`, `utils.aoa_to_sheet`, `utils.book_new`/`book_append_sheet`, `writeFile`, `utils.encode_col`/`encode_cell`) is a long-stable core API untouched by this jump, confirmed with a round-trip test (write a sheet, read it back, encode a cell) against the live page -- numbers and strings came back exactly as written.
- **jsPDF**: 2.5.1 to 4.2.1. Used in exactly one place (`canvasToPdfBlob()` in `js/app.js`, the WhatsApp-share PDF path) via the same stable `new jsPDF(...)` / `addImage` / `addPage` / `output('blob')` surface across both versions -- tested with a synthetic canvas through the same code path, produced a valid PDF blob.
- **exceljs** (4.4.0) and **html2canvas** (1.4.1) turned out to already be the current release -- no change needed, despite exceljs's minified header only showing a build date rather than a version number.
- **@azure/msal-browser** (3.30.0) and **pdf.js** (3.11.174, `pdf.min.js`/`pdf.worker.min.js`) were deliberately left alone this round. Both are two major versions behind, but both upgrades carry real breaking-change risk that can't be safely verified in this environment: msal-browser 5.x's changelog documents removed/renamed APIs around `PublicClientApplication`, and this app's OneDrive login can only be truly tested against a live Microsoft account, not headlessly. pdf.js 4.x+ dropped the classic `<script src>`/global-`pdfjsLib` build entirely in favor of ES modules only -- upgrading it means rewriting how `tools/passsheet.html` and `tools/hbr-report.html` load it, not just swapping a file. Flagging both as follow-up work rather than risking a live break to OneDrive sign-in or the PassSheet/HBR PDF-splitting tools on a guess.
...
Fixed by scaling every text and line element on the poster page for its true physical size, not its on-screen appearance: branch name labels 15/16pt to 26/30pt, the title 46pt to 72pt (with the header strip widened to fit), context-city labels switched from `Helvetica-Oblique` to bold-oblique at larger sizes, legend/stats/footer text all bumped roughly 40-70%, and every stroke (district boundaries, major/minor roads, map frame, pin outlines, label leader lines and label-box borders) thickened 1.5-2x. The three pale colors doing most of the damage -- muted ink, minor-road grey, and the divider-line grey -- were all darkened (`#5C6D64`→`#38443E`, `#B9BFB8`→`#8B948D`, `#D7DED9`→`#AAB4AC`) so they hold real ink coverage instead of washing out.

None of this touched the guaranteed-placement logic itself, just the sizes it plans around (bigger padding, bigger search step, since the label boxes are now bigger too) -- still all 57 of 57 branches get a placed name with no drops, 7 needed a leader line this time (up from 3) since the bigger label boxes collide more often in the tight Hathras-town cluster, verified by re-rendering the page to a PNG and inspecting that cluster up close: every name legible and its leader line correctly pointing back to its own pin. Link annotation count re-verified unchanged at 171 after the rebuild and merge with the (untouched) directory table pages.

### Follow-up: route panel now supports multi-stop itineraries, not just 2 branches (2026-09-15, same day)

Alok's next ask: "is route main agar branches add karni ho like main aaj 3 branch visit ki to uska route kaisa banaun" -- a day where he visits more than 2 branches, with the combined by-road route and total distance across all of them in visit order, not just point-to-point. Generalized the fixed From/To route panel into a dynamic stops list: a numbered row per stop (badge, SOL-ID-sorted branch dropdown, remove button), a "+ Add another branch" button, 2-8 stops, order = the order rows are added in (no drag-reorder -- removing and re-adding a stop covers the rare case where the visit order needs to change). Toolbar button and panel title reworded from "Route between 2 branches" to "Plan a route" since it no longer hardcodes a count; the detail card's "Route from here to…" quick action still works, now pre-filling stop 1 of the list.

The OSRM request generalizes for free -- a single `/route/v1/driving/lon1,lat1;lon2,lat2;lon3,lat3...` call with any number of semicolon-separated waypoints returns one stitched route with `route.legs`, a per-leg distance/duration array, alongside the usual `route.distance`/`route.duration` totals, confirmed via `curl` against the live endpoint (3 waypoints -> 2 legs, correct per-leg and total figures). So a 3+ stop itinerary still costs exactly one network call. The result panel now shows the total distance/drive time as before, plus a per-leg breakdown line ("R O Hathras -> Agsauli: 2.8 km", "Agsauli -> Bamnai: 13.7 km") and the summed straight-line distance for comparison. Route focus mode (hide every branch not on the route) and the graceful straight-line fallback on a routing failure both generalized to any stop count with no behavior change for the existing 2-stop case.

Verified locally with a mocked 3-waypoint/2-leg OSRM response (this sandbox's outbound proxy still can't reach the real `router.project-osrm.org` for an end-to-end check, same documented limitation as the original 2-branch route): adding a 3rd stop, selecting 3 branches, and clicking "Show route" correctly rendered the total distance, both per-leg lines, and 3 numbered stop markers, while 54 of the other 57 branch pins hid via focus mode; Clear correctly reset back to 2 empty stops with every pin restored.
...
The routing itself comes from OSRM's free public demo server (`router.project-osrm.org`) -- no API key, and it already serves proper CORS headers for direct browser `fetch()`, confirmed with `curl -I` against the live endpoint. It's a shared demo instance with no uptime guarantee, so a failed/slow response shows a plain "route lookup failed, here's the straight-line distance instead" message rather than leaving the panel looking broken.

This one shipped with a real testing gap worth being honest about: this sandbox's own outbound-network proxy setup blocks headless Chromium's `fetch()` calls to `router.project-osrm.org` before they even reach the proxy's relay layer (confirmed via the proxy's own status endpoint -- no record of the request arriving at all, unlike a normal blocked/failed request), so the actual route-lookup call couldn't be exercised end-to-end in this environment. Everything that *could* be tested was: the panel toggle, dropdown population (all 57 branches), the Go button's enabled/disabled gating, the swap button, the detail-card quick-action pre-fill, and the clear button -- all verified working with no console errors. The network call itself needs a real check once live, since a sandboxed dev environment isn't proof a production browser will succeed, even with the API and its CORS headers independently confirmed correct via `curl`.
...
Rebuilt the map page from scratch as **pure vector artwork** instead -- drawn directly with `reportlab`'s canvas primitives (paths, circles, text), not a raster image at all. Real OpenStreetMap road geometry (fetched earlier this session via Overpass, re-simplified here in lat/lon rather than the projected units used for the live map's SVG) and the district boundary polygons are both drawn as vector paths; branch pins are vector circles; every name is real PDF text. None of it can pixelate at any print size, because a PDF viewer or RIP re-rasterizes vector content fresh at whatever resolution the final output needs -- printed at 6x4 feet, at 60 feet, or viewed at 400% zoom on a phone, the line weights and text stay exactly as sharp as designed. The page itself is sized at a literal 72in x 48in (6ft x 4ft) so it can be printed at 100% with no scaling math needed.

The real fix for "some names aren't showing," though, wasn't resolution -- it was the label-placement algorithm. Every one of the 57 branches now gets a guaranteed name: each tries a ring of nearby positions first, and if the immediate area is too crowded (6 of the branches sit within 2km of each other in Hathras town), the search radius widens step by step across the whole page until it finds open space, at which point a thin leader line is drawn back to the actual pin so it still reads as "this name belongs to that dot" rather than floating free. Only 3 of the 57 needed a leader line in practice -- the real-world geographic scale at 6x4 feet turned out to give enough natural separation for the other 54, even inside the Hathras-town cluster. This guarantees no branch is ever silently dropped, unlike the earlier collision-avoidance pass (skip the label if it overlaps) which is what caused the missing names in the first place.

Footer now reads "Developed by Recovery Team Hathras" on every page (poster included) per Alok's request. The poster page and the SOL-ID-sorted directory tables are built as two separate PDFs (`reportlab` doesn't mix page sizes in one `SimpleDocTemplate` easily) and merged into the one shipped file with `pypdf` -- which needed `cffi`/`cryptography` reinstalled first; the versions already on this image had a broken native-extension chain that crashed pypdf's import with a Rust panic before any of this code even ran.
...
Built as a new seventh Utility Hub tool, `tools/branch-map.html`, wired in the same way as the others (card in the Utility Hub grid, `#viewBranchMap` section, `branchmap` added to `UTILITY_CHILD_VIEWS`) but with one deliberate difference from its siblings: it uses the *default* `.tool-frame-wrap` sizing (PassSheet's pattern -- fixed viewport height, internal scroll), not the auto-height override every other tool needs. This is a fixed-viewport app (map + sidebar panel), not a growing table, so the override that fixed NPA SOL Summary's scroll bug would be the wrong pattern here.

What's on the map:
- **Roads**: real OpenStreetMap road geometry (motorway/trunk/primary/secondary, ~1,970 ways) for Hathras + Mathura districts, fetched via the Overpass API, simplified with the same Ramer-Douglas-Peucker code already used for the district boundaries, and drawn as SVG paths with a casing+fill pair per class (Google Maps-style double-line look). First attempt rendered nothing -- `vector-effect:non-scaling-stroke` measures `stroke-width` in real CSS pixels, decoupled from the viewBox's own unit scale, so the initial fractional widths (`.002` etc., copied from the pins' convention) were sub-pixel and invisible; fixed by using literal pixel values (1-5px) instead.
- **On-map name labels**: an HTML overlay (not SVG text, so font-size stays fixed on screen regardless of zoom) with a real screen-space collision/declutter pass re-run on every pan and zoom -- Regional Office always wins a clash, everything else yields, so labels never overlap and more of them appear as you zoom in, the same behaviour a tile-based map gives for free.
- **"Open in Google Maps"**: every branch's detail card gets a link to `google.com/maps/search/?api=1&query=<lat>,<lon>` for real turn-by-turn directions -- something no amount of custom SVG can substitute for.
- **Exact GPS from the branch photos**: 55 of 57 positions now come from the GPS burned into each branch's own site photo (via `GPS Map Camera`-style overlays), not the registered-address geocoding used before. Most photos print a precise decimal lat/lon directly; a handful only printed a rounded value plus a Google Plus Code, recovered to full precision with the official `openlocationcode` library (fetched as a PyPI sdist since the wheel build failed in-sandbox) using the photo's own rounded reading as the recovery reference point. One upload (`9316 Maant.jpeg`) turned out to be a duplicate of the Laxmi Nagar photo -- same GPS to 7 decimal places, and the branch signboard visible in the shot reads "LAXMI NAGAR," not "Maant" -- so that one branch was deliberately left on its previous address-based position rather than trust the mismatched photo; R O Hathras (no photo taken) is the other. Both are flagged with the existing dashed-ring "approximate" marker, everything else lost it.

A live map tile provider (Leaflet, Google Maps embed) was ruled out from the start -- external images/fetches are blocked in the standalone-Artifact sandbox this was prototyped in, and even once it shipped as a real same-origin iframe tool where that restriction no longer applies, staying with the inline-SVG approach kept the tool self-contained and offline-capable like every other Utility tool, with the Google Maps link covering the one thing a real map does better (live traffic-aware directions).

### Follow-up: replaced the hand-drawn map with a real Leaflet map, tiles and all (2026-09-15, same day)

Alok's next request made the earlier trade-off moot: "proper Google Map ki tarah dikhna chahiye and near by area bhi" -- he wanted the actual surrounding geography (neighbouring towns, real street detail beyond just Hathras/Mathura's own two districts), which no amount of hand-drawn SVG can honestly provide. Once this tool is a real same-origin page in the app rather than an Artifact preview, there's no CSP reason left to avoid live map tiles, so `tools/branch-map.html` was rebuilt around **Leaflet** (loaded from cdnjs) instead of custom SVG pan/zoom.

- **Base map**: standard OpenStreetMap raster tiles (`tile.openstreetmap.org`), which show the real road network, neighbouring towns/districts, rivers, and place names for free with no API key. CARTO's Positron/Voyager/Dark Matter tiles were tried first (same URLs that worked mid-session for the district-boundary lookups) but now silently return a 256×256 "API KEY REQUIRED" placeholder image with an HTTP 200 -- caught only by actually opening a fetched tile, not by trusting the status code -- so they were dropped in favour of OSM's own tiles, which have no separate dark style; dark theme instead applies a CSS `invert(1) hue-rotate(180deg)` filter to the tile pane, a common and honest approximation rather than a real dark basemap.
- **Real "nearby area" context** replaces the hand-drawn district polygons + Overpass-fetched road paths from the previous version entirely -- both are now dead weight since the tiles themselves show far more real detail (every street, not just motorway/trunk/primary/secondary) at every zoom level, not just within the two districts' own boundaries. The district polygons stay on top as a light, semi-transparent overlay (still useful for "which district is this branch in" at a glance); the custom road-drawing code is gone.
- **Branch name labels are now real Leaflet tooltips** (`bindTooltip(..., {permanent:true, interactive:true})`) instead of a hand-rolled HTML-overlay/collision-detection pass -- simpler, and Leaflet repositions them automatically on every pan/zoom for free. Declutter is a single zoom-level threshold (labels other than the Regional Office and whichever branch is selected only render above zoom 12) rather than full collision detection, since the real tiles already give enough spatial context to disambiguate a cluster by zooming in, the way an actual map app expects you to.
- **"Branch name par click karne par right side main uski information"**: clicking a name tooltip directly on the map -- not just the sidebar list -- now selects that branch (`marker.getTooltip().on('click', ...)`), flies the map to it, and opens the same detail card + "Open in Google Maps" link as clicking its sidebar row. Verified via Playwright: clicking the "Tuksan" label on the map populates the detail card with Tuksan's own address/PIN/email, not the previously-selected branch's.
- First attempt broke silently for an unrelated reason: our own script tag sat correctly *after* `<script src=".../leaflet.min.js">` in document order, but Playwright reported `L is not defined` regardless -- turned out to be `ERR_CERT_AUTHORITY_INVALID` on cdnjs/tile requests, a sandbox-only side effect of this environment's intercepting HTTPS proxy using a CA Chromium doesn't trust by default (documented elsewhere in this environment, unrelated to the code) -- confirmed harmless for real users by re-testing with `ignoreHTTPSErrors` in the test harness only, never shipped that way.

### Bug fix: restored the table's own scroll region, which brought its sticky header back too (2026-09-14, same day)

Alok's next screenshot (scrolled deep into the table, desktop) showed the "SOL ID / Branch Name / ..." header gone -- scrolled away with the rows instead of staying pinned. Root cause: dropping `.table-wrap`'s `max-height:480px;overflow:auto` two commits ago (done because a nested-scroll conflict looked like a plausible explanation for "scroll nahi ho raha," before the real cause -- the missing `#viewNpaSolSummary .tool-frame-wrap` override -- was found and fixed independently) took away the only scrolling context inside this iframe's own document. `position:sticky` only sticks relative to a scrolling ancestor in the *same* document; once nothing inside the iframe scrolled internally anymore (it's sized to fit all its content, and the actual scroll happens on the *parent* page, a different browsing context CSS positioning can't reach into), the header had nothing to stick to and just scrolled normally with everything else.

Restored `.table-wrap{max-height:480px;overflow:auto}` to match HBR Report / NPA Region Summary / Daily PNPA Summary exactly -- the nested-scroll theory that justified removing it was never actually confirmed and turned out to be a red herring; the real bug was already fixed elsewhere by then, so the table can safely go back to the same working pattern every sibling tool already uses. Verified via Playwright: the header's rendered position is now identical before and after scrolling the table 1,341px, staying pinned to the table's own top edge exactly as before.

### Follow-up: the actual scroll bug, a Crore total, and a plain-magnitude Debit figure (2026-09-14, same day)

The table-wrap scroll fix two commits ago turned out not to be the bug Alok was still hitting -- his next screenshot (desktop, not mobile) showed the page cut off mid-table with no scrollbar anywhere. The real cause: every other self-resizing Utility tool (HBR Report, Daily PNPA Summary, NPA Region Summary, Telephone Directory) has its own `#viewXxx .tool-frame-wrap{height:auto;...}` override in css/styles.css, because the *default* `.tool-frame-wrap` rule is `height:calc(100vh - 210px);overflow:hidden` -- a fixed viewport-relative height with no scrollbar at all, meant for tools that manage their own internal scroll (PassSheet). NPA SOL Summary never got that override added when it shipped, so its wrapper was silently clipping the table at a fixed height with nothing to scroll -- not a touch-specific nested-scroll conflict this time, just a missing three lines of CSS. Added `#viewNpaSolSummary .tool-frame-wrap`/`.tool-frame` matching the sibling tools exactly.

Two more requests from the same round: Debit Balance now displays as a plain magnitude (no minus sign) -- the source report's negative-for-outstanding-debit convention is an internal ledger quirk that added nothing on a summary built specifically to show that figure, so `fmtLakh()`/`fmtCrore()` both take `Math.abs()` before formatting (sorting and the report's-own-total integrity check still use the real signed `rec.dr` underneath, unaffected). And a third stat tile, "Total Debit Balance (₹ Cr)", added next to Accounts Read/Branches at the top of the page -- the whole-file total at a glance, in Crore rather than Lakh since a region-wide NPA figure reads better a step up in scale from a single branch's.
